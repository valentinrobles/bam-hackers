...
const require_workspace = require("./workspace-BmarCUep.cjs");
const require_tools = require("./tools-BPNoTnQQ.cjs");
const require_channels = require("./channels-Pck0LAjG.cjs");
const require_mastra = require("./mastra-CpEoV-tp.cjs");
let crypto = require("crypto");
let zod_v4 = require("zod/v4");
let _mastra_schema_compat_schema = require("@mastra/schema-compat/schema");
//#region src/agent-controller/message-author.ts
function readMessageAuthor(requestContext) {
	const value = requestContext?.get(require_request_context.MASTRA_MESSAGE_AUTHOR_KEY);
...
function withMessageAuthor(providerOptions, author) {
	if (!author) return providerOptions;
	return {
		...providerOptions,
		mastra: {
			...providerOptions?.mastra,
			author
		}
	};
}
...
var SessionRunEngine = class {
	...
	*/
	...
	isCurrentMessageObserved(state) {
	...
	createStreamState() {
		return {
			...
			isSuspended: false,
			spans: new require_agent.MessagePartSpans({ providerMetadata: false }),
	...
	*/
	async processStreamChunk(state, chunk, requestContext, agent = this.#machinery.getAgent()) {
		if ("runId" in chunk && chunk.runId) this.#session.run.setRunId({ runId: chunk.runId });
		if (require_agent.isSpanChunk(chunk)) {
			const folded = state.spans.fold(state.currentMessage.content.parts, chunk);
			if ((chunk.type === "text-start" || folded?.created && folded.part.type === "text") && !state.messageIdObserved) {
		...
		switch (chunk.type) {
			case "step-start": {
				...
				if (state.offeredResponseIds.size > 0 || state.completedToolPrelude && state.currentMessage.content.parts.every((part) => part.type === "tool-invocation" && part.toolInvocation.state === "result")) this.finishCurrentMessageAndRotate(state);
				state.completedToolPrelude = false;
				state.offeredResponseIds.add(messageId);
				if (!this.isCurrentMessageObserved(state)) state.currentMessage.id = messageId;
				break;
			}
			...
			case "tool-call-suspended": {
				...
				if (suspRunId) {
					...
					if (!runScope?.get(SUSPENDED_RUN_AGENT_KEY)) runScope?.set(SUSPENDED_RUN_AGENT_KEY, agent);
			...
			case "step-finish": {
				state.completedToolPrelude = state.offeredResponseIds.size === 0 && this.hasCurrentMessageContent(state) && state.currentMessage.content.parts.every((part) => part.type === "tool-invocation" && part.toolInvocation.state === "result");
				...
				if (usageRecord) {
					...
					const rawCompletion = getUsageNumber(usageRecord, "completionTokens") ?? getUsageNumber(usageRecord, "outputTokens");
					const rawTotal = getUsageNumber(usageRecord, "totalTokens");
					if (rawPrompt !== void 0 || rawCompletion !== void 0 || rawTotal !== void 0) {
	...
	*/
	...
	retractFailedRunSuspensions({ runId, reason }) {
		if (!runId) return;
		for (const { toolCallId, toolName } of this.#session.suspensions.deleteForRun({ runId })) this.#session.emit({
	...
	async handleSubscribedStreamError(error) {
		if (error instanceof Error && error.name === "AbortError") await this.#session.finishAgentRun("aborted");
			...
			const streamError = require_error.getErrorFromUnknown(error);
			this.#session.emit({
				type: "error",
				error: streamError
			});
...
const SUSPENDED_RUN_AGENT_KEY = require_agent.createRunScopeKey("agent-controller.suspendedRunAgent");
...
var SessionThread = class {
	...
	*/
	async ensureSubscription(threadId, agent = this.#owner.machinery.getAgent()) {
...
var Session = class {
	...
	#preparingFollowUps = /* @__PURE__ */ new Set();
	...
	*/
	...
	async waitForAcceptedRunCompletion(accepted, { waitForDelivery = true } = {}) {
		const completedRunIds = /* @__PURE__ */ new Set();
		let runId;
		...
		try {
			...
			runId = "runId" in result ? result.runId : void 0;
	...
	*/
	...
	sendSignalToThread(input, target) {
		const signal = require_signals.createSignal(input);
		const accepted = Promise.resolve().then(async () => {
			const resourceId = this.identity.getResourceId();
			const thread = target.resourceId === resourceId ? await this.thread.getById({ threadId: target.threadId }) : null;
			if (!thread || thread.resourceId !== resourceId) throw new Error(`Thread not found: ${target.threadId}`);
			const result = this.machinery.getAgent().sendSignal(signal, {
				...target,
				ifActive: { behavior: "persist" },
				ifIdle: { behavior: "persist" }
			});
			if ((await result.accepted).action === "persist") {
	...
	*/
	sendSignal(input, options) {
		...
		const ifActive = options?.ifActive ?? ("content" in input ? input.ifActive : void 0);
		const ifIdle = options?.ifIdle ?? ("content" in input ? input.ifIdle : void 0);
		...
		const accepted = Promise.resolve().then(async () => {
			...
			if (!submittedAbortRequested && submittedRunId && submittedActiveRunId && submittedIsRunning) {
				if (signal.type === "user") this.approval.respond({
					decision: "decline",
					declineContext: {
				...
				const settled = ifActive?.behavior === "persist" || ifIdle?.behavior === "persist" || requireDelivery ? await result.accepted : void 0;
				...
				if (requireDelivery) {
					...
					return {
						accepted: true,
						runId: "runId" in acceptedResult ? acceptedResult.runId : void 0,
			...
			if (submittedAbortRequested && (submittedRunId || submittedActiveRunId)) {
				if (!await this.waitForStreamIdle()) this.thread.cleanupSubscription();
				await this.thread.ensureSubscription(threadId, agent);
			}
			...
			if (ifIdle?.behavior === "persist") await result.persisted;
			return {
				accepted: true,
				runId: void 0
			};
	...
	*/
	...
	async prepareMessageTarget({ requestContext, tracingContext, tracingOptions, untilIdle, includeStreamOptions = true }) {
		if (!this.thread.getId()) {
			const thread = await this.thread.create();
			this.thread.set({ threadId: thread.id });
		}
		const threadId = this.thread.getId();
		await this.thread.ensureSubscription(threadId);
		if (!includeStreamOptions) return {
			resourceId: this.identity.getResourceId(),
			threadId
		};
		const streamOptions = await this.machinery.buildStreamOptions({
	...
	*/
	async sendMessage({ content, files, tracingContext, tracingOptions, requestContext: requestContextInput, untilIdle }) {
		const wasActive = this.stream.isActive();
		const signal = this.sendSignal({
			content: this.createMessageInput({
				content,
				files
			}),
			tracingContext,
		...
		else await this.waitForAcceptedRunCompletion(signal.accepted);
	}
	/**
	...
	*/
	async queueMessage({ content, files, tracingContext, tracingOptions, requestContext: requestContextInput }) {
		const wasActive = this.stream.isActive();
		...
		const providerOptions = withMessageAuthor(void 0, readMessageAuthor(requestContextInput));
		const result = this.machinery.getAgent().queueMessage(providerOptions ? {
			contents: messageInput,
			providerOptions
		} : messageInput, target);
		if (wasActive) await result.accepted;
		else await this.waitForAcceptedRunCompletion(result.accepted, { waitForDelivery: false });
	}
	...
	ensureFollowUpBinding(agent, resourceId, threadId) {
		const existing = this.#followUpBinding;
		if (existing?.agent === agent && existing.resourceId === resourceId && existing.threadId === threadId) return existing;
		...
		try {
			const unsubscribe = agent.subscribeThreadEvents({
				resourceId,
				threadId
			}, (event) => {
				if (event.type === "queue-count-changed" && this.#followUpBinding === binding) this.emit({
	...
	async followUp({ content, requestContext }) {
		...
		const resourceId = this.identity.getResourceId();
		const agent = this.machinery.getAgent();
		if (!this.ensureFollowUpBinding(agent, resourceId, threadId)) return;
		const operation = {
			generation: this.#followUpGeneration,
		...
		try {
			...
			await agent.queueMessage({
				...
				providerOptions: withMessageAuthor(void 0, readMessageAuthor(requestContext))
			}, {
				resourceId,
				threadId,
				ifIdle: { streamOptions }
	...
	*/
	...
	createSubscribedResumeBoundaryWaiter({ toolCallId, resolveOnToolEnd = false }) {
		let unsubscribe;
		return {
	...
	*/
	async resumeToolCall({ resumeData, toolCallId, requestContext: requestContextInput, resolveOnToolEnd = false }) {
		const suspension = this.suspensions.get({ toolCallId });
		if (!suspension) throw new Error("No active suspension to resume");
		const agent = this.machinery.getRunScope(suspension.runId)?.get(SUSPENDED_RUN_AGENT_KEY) ?? this.machinery.getAgent();
...
function createSubagentTool(opts) {
	...
	if (subagents.length === 0) throw new Error("createSubagentTool requires at least one subagent");
...
var AgentController = class {
	...
	*/
	#wireSession(session) {
		...
		session.setMachinery({
			...
			subscribeToThread: ({ agent, resourceId, threadId }) => (agent ?? this.getCurrentAgent(session)).subscribeToThread({
				resourceId,
				threadId
			}),
	...
	* workspace/sandbox.
	*/
	async queryThreadMessages({ threadId, resourceId, perPage, page, orderBy = {
		field: "createdAt",
		direction: "DESC"
	}, include, filter }) {
		await this.initStorage();
		...
		return {
			...result,
			messages: result.messages.map((msg) => this.convertToControllerMessage(msg))
		};
	}
	async queryFirstUserMessages({ threadIds }) {
		...
		for (const message of result.messages) {
			if (!require_signals.isUserAuthoredMessage(message) || !message.threadId || firstUserMessages.has(message.threadId)) continue;
	...
	*/
	async generateThreadTitle({ threadId, resourceId, scope, model, requestContext: callerContext }) {
		...
		const messages = new require_message_list.MessageList().add(recent.messages.reverse(), "memory").get.all.ui();
		if (!messages.some((message) => message.role === "user")) throw new Error("This conversation has no message to name it from yet.");
		const session = resourceId ? await this.getSessionByResource(resourceId, scope) : void 0;
		const agent = session ? this.getCurrentAgent(session) : this.propagateRuntimeServicesToAgent(this.getAgentForMode(this.#defaultMode));
		const requestContext = session ? await this.buildRequestContext(session, callerContext) : callerContext ?? new require_request_context.RequestContext();
		const configured = (await agent.getMemory({ requestContext }))?.getMergedThreadConfig().generateTitle;
		const titleConfig = typeof configured === "object" ? configured : void 0;
	...
	*/
	...
	async buildAgentMessageStreamOptions({ session, requestContext: requestContextInput, tracingContext, tracingOptions, untilIdle, abortSignal }) {
		const runThreadId = session.thread.getId();
		if (!runThreadId) throw new Error("Cannot build stream options without a current thread");
		const resourceId = session.identity.getResourceId();
		const modeId = session.mode.get();
		if (!abortSignal) session.run.clearAbortRequested();
		...
		const streamOptions = {
			...
			memory: {
				thread: runThreadId,
				resource: resourceId,
				onTitleGenerated: (title) => session.emit({
					type: "thread_title_updated",
					threadId: runThreadId,
					title
				})
		...
		if (this.config.subagents?.length) {
			...
			builtInTools.subagent = createSubagentTool({
				...
				cloneThreadForFork: hasMemory ? async ({ sourceThreadId, resourceId, title }) => {
					const result = await (await this.resolveMemory(session)).copyThread({
						sourceThreadId,
	...
	*/
	async buildRequestContext(session, requestContext, scope) {
		requestContext = new require_request_context.RequestContext(requestContext?.entries());
...
//# sourceMappingURL=agent-controller-C1klqGRQ.cjs.map