...
var BackgroundTaskManager = class {
	pubsub;
	workerId = randomUUID();
	processAffineDispatchTopic = `${TOPIC_DISPATCH}:${this.workerId}`;
	config;
	#mastra;
	...
	localReservations = /* @__PURE__ */ new Map();
	localPendingTaskIds = /* @__PURE__ */ new Set();
	...
	async #doInit(pubsub) {
		...
		if (!isProducerOnly) {
			...
			if (this.#mastra) {
				const { buildBackgroundTaskWorkflow } = await import("./workflow-Diqxp4fc.js");
			...
			if (this.shuttingDown) {
				await this.#releaseLateInitSubscriptions([[TOPIC_DISPATCH, this.workerCallback], [this.processAffineDispatchTopic, this.workerCallback]]);
				return;
			}
			if (this.config.recoverStaleTasksOnStart) await this.recoverStaleTasks();
		}
	...
	*/
	async enqueue(payload, context) {
		...
		if (Boolean(context) ? this.reserveLocalSlot(task) : await this.checkConcurrency(task.agentId)) {
			try {
				await this.dispatch(task);
			} catch (error) {
				if (await storage.updateTask(task.id, {
					status: "failed",
	...
	async cancel(taskId) {
		...
		while (true) {
			...
			if (!await storage.updateTask(taskId, {
				status: "cancelled",
				completedAt: /* @__PURE__ */ new Date()
			...
			if (cancelledTask) {
				...
				try {
					...
					this.#mastra?.getLogger?.()?.warn(`background-task cancellation callback failed for ${taskId}:`, error);
				}
			}
	...
	*/
	async resume(taskId, resumeData) {
		...
		const isProcessAffine = this.taskContexts.has(taskId);
		if (!(isProcessAffine ? this.reserveLocalSlot(task) : await this.checkConcurrency(task.agentId))) throw new Error(`Concurrency limit reached, cannot resume task "${taskId}" — retry once a slot is available`);
	...
	*/
	async restart(taskId, context) {
		...
		if (context) this.registerTaskContext(task.id, context);
		const isProcessAffine = this.taskContexts.has(taskId);
		if (!(isProcessAffine ? this.reserveLocalSlot(task) : await this.checkConcurrency(task.agentId))) throw new Error(`Concurrency limit reached, cannot restart task "${taskId}" — retry once a slot is available`);
		try {
	...
	*/
	async waitForNextTask(taskIds, options) {
		...
		return new Promise((resolve, reject) => {
			...
			const handleAbort = () => {
				cleanup();
				reject(options?.abortSignal?.reason ?? /* @__PURE__ */ new Error("Background task wait aborted"));
			};
			...
			progressInterval = options?.onProgress ? setInterval(() => {
	...
	*/
	async #waitForShutdownStep(description, promise) {
		...
		if (outcome.status === "timed-out") {
			this.#mastra?.getLogger?.()?.warn(`${description} exhausted the remaining ${remainingMs}ms of the graceful shutdown budget`);
			return;
		}
	...
	*/
	async handleDispatch(event) {
		...
		if (!(isRestart ? task.status === "running" : task.status === "pending" || isRunningRedelivery)) return true;
		const retryCount = task.status === "running" ? deliveryAttempt - 1 : task.retryCount;
		if (!await storage.updateTask(taskId, {
			status: "running",
			startedAt: /* @__PURE__ */ new Date(),
			retryCount
		}, { expectedStatus: task.status })) return true;
		if (this.shuttingDown) return false;
	...
	*/
	...
	async handleResult(event) {
		...
		try {
			if (event.type === "task.completed") {
				...
				await Promise.all([ctx?.onComplete?.(task), (async () => {
					try {
						await this.config.onTaskComplete?.(task);
					} catch (error) {
						this.#mastra?.getLogger?.()?.warn(`background-task completion callback failed for ${taskId}:`, error);
					}
				})()]);
			}
			if (event.type === "task.failed") {
				...
				await Promise.all([ctx?.onFailed?.(task), (async () => {
					try {
						await this.config.onTaskFailed?.(task);
					} catch (error) {
						this.#mastra?.getLogger?.()?.warn(`background-task failure callback failed for ${taskId}:`, error);
					}
				})()]);
			}
		} finally {
	...
	reserveLocalSlot(task) {
		...
		let agentRunning = 0;
		for (const agentId of this.localReservations.values()) if (agentId === task.agentId) agentRunning++;
	...
	async drainPending() {
		...
		try {
			...
			for (const taskId of [...this.localPendingTaskIds]) {
				...
				const isProcessAffine = this.taskContexts.has(taskId);
				if (!(isProcessAffine ? this.reserveLocalSlot(task) : await this.checkConcurrency(task.agentId))) continue;
				this.localPendingTaskIds.delete(taskId);
				try {
					await this.dispatch(task);
				} catch (error) {
					...
					this.#mastra?.getLogger?.()?.warn(`background-task dispatch failed while draining ${taskId}:`, error);
					return;
				}
			}
		} catch (error) {
			this.#mastra?.getLogger?.()?.warn("background-task queue drain failed:", error);
		} finally {
...
function resolveBackgroundConfig({ llmBgOverrides, toolName, toolConfig, agentConfig, managerConfig }) {
	...
	const defaultDisposition = resolveDefaultDisposition({
		toolName,
		toolConfig,
		agentConfig
	});
	const requestedDisposition = llmOverride?.disposition ?? (llmOverride?.enabled === false ? "foreground" : llmOverride?.enabled === true ? "deferred" : defaultDisposition);
	const disposition = baseEnabled ? requestedDisposition : "foreground";
...
*/
function isToolBackgroundEligible({ toolName, toolConfig, agentConfig }) {
	if (agentConfig?.disabled) return false;
	return resolveAgentToolConfig(toolName, agentConfig)?.enabled ?? toolConfig?.enabled ?? false;
}
/**
...
* cannot disagree.
*/
function resolveDefaultDisposition({ toolName, toolConfig, agentConfig }) {
	return resolveAgentToolConfig(toolName, agentConfig)?.defaultDisposition ?? toolConfig?.defaultDisposition ?? "deferred";
}
...
const backgroundOverrideJsonSchema = {
	type: "object",
	description: "Optional: override how this specific call executes (foreground, deferred, or awaited). When omitted, the tool's configured default disposition applies. The enabled boolean remains supported for compatibility.",
	properties: {
		enabled: {
			type: "boolean",
...
const backgroundOverrideZodSchema = z.object({
	...
	disposition: z.enum([
		...
		"deferred",
		"awaited"
	]).optional().describe("Choose foreground, deferred, or awaited execution for this call."),
...
}).optional().describe("Optional: override how this specific call executes (foreground, deferred, or awaited). When omitted, the tool's configured default disposition applies. The enabled boolean remains supported for compatibility.");
//#endregion
...
function generateBackgroundTaskSystemPrompt(tools, agentConfig) {
	...
	for (const [toolName, tool] of Object.entries(tools)) {
		...
		if (isToolBackgroundEligible({
			toolName,
			toolConfig,
			agentConfig
		})) eligibleTools.push({
			toolName,
			defaultDisposition: resolveDefaultDisposition({
				toolName,
				toolConfig,
				agentConfig
			})
...
${eligibleTools.map(({ toolName, defaultDisposition }) => defaultDisposition === "deferred" ? `- ${toolName} (default: background)` : `- ${toolName} (default: foreground — opt in with "_background")`).join("\n")}
...
//#endregion
export { resolveBackgroundConfig as a, BACKGROUND_TASK_WORKFLOW_ID as c, isToolBackgroundEligible as i, backgroundOverrideJsonSchema as n, createBackgroundTask as o, backgroundOverrideZodSchema as r, BackgroundTaskManager as s, generateBackgroundTaskSystemPrompt as t };

//# sourceMappingURL=background-tasks-Dt8uUqZI.js.map