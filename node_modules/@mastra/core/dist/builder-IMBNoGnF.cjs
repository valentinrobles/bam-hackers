...
const ENTRY_ID_DESCRIPTION = "Optional stable entry id — kebab-case, unique within the workflow. Lets editors and tools address this control-flow entry across edits and serialization.";
const ENTRY_DESCRIPTION_DESCRIPTION = "Optional human-readable description of why this control-flow operation exists.";
const ENTRY_METADATA_DESCRIPTION = "Arbitrary JSON-safe metadata attached to this entry (e.g. a display title for visual editors). Does not affect execution.";
const entryDisplayFields = {
	description: zod.z.string().optional().describe(ENTRY_DESCRIPTION_DESCRIPTION),
	metadata: jsonSchema.optional().describe(ENTRY_METADATA_DESCRIPTION)
};
const entryDisplayInputFields = {
	description: zod.z.string().nullish().describe(ENTRY_DESCRIPTION_DESCRIPTION),
	metadata: zod.z.unknown().nullish().describe(ENTRY_METADATA_DESCRIPTION)
};
const containerIdentityFields = {
	id: zod.z.string().min(1).optional().describe(ENTRY_ID_DESCRIPTION),
	...entryDisplayFields
};
const containerIdentityInputFields = {
	id: zod.z.string().min(1).nullish().describe(ENTRY_ID_DESCRIPTION),
...
const workflowBuilderSleepEntryInputSchema = zod.z.strictObject({
	type: zod.z.literal("sleep"),
	id: zod.z.string().min(1),
	...entryDisplayInputFields,
	duration: zod.z.number().nonnegative().describe("Milliseconds to wait. Static number only.")
});
const workflowBuilderSleepUntilEntryInputSchema = zod.z.strictObject({
	type: zod.z.literal("sleepUntil"),
	id: zod.z.string().min(1),
	...entryDisplayInputFields,
	date: zod.z.string().min(1).describe("ISO 8601 wall-clock date to wait until. Static string only.")
...
const SCHEDULE_DESCRIPTION = "Optional declarative cron schedule(s) for the workflow. A single config or an array (array entries must each provide a unique stable id). Persisted with the definition and re-registered on every boot.";
const workflowBuilderJsonValueSchema = zod.z.lazy(() => zod.z.union([
	zod.z.null(),
	zod.z.string(),
	zod.z.number().finite(),
	zod.z.boolean(),
	zod.z.array(workflowBuilderJsonValueSchema),
	zod.z.record(zod.z.string(), workflowBuilderJsonValueSchema)
]));
const workflowBuilderScheduleConfigSchema = zod.z.strictObject({
	id: zod.z.string().min(1).optional().describe("Stable schedule id, scoped to the workflow. Required in array form."),
	cron: zod.z.string().min(1).describe("Cron expression (5-, 6-, or 7-part)."),
	timezone: zod.z.string().min(1).optional().describe("Optional IANA timezone."),
	...
	requestContext: zod.z.record(zod.z.string(), workflowBuilderJsonValueSchema).optional().describe("Request context for each scheduled run."),
	metadata: zod.z.record(zod.z.string(), workflowBuilderJsonValueSchema).optional().describe("Metadata persisted on the schedule row.")
}).superRefine(({ cron, timezone }, ctx) => {
	try {
		require_cron.validateCron(cron, timezone);
	} catch (error) {
...
const workflowBuilderScheduleSchema = zod.z.union([workflowBuilderScheduleConfigSchema, zod.z.array(workflowBuilderScheduleConfigSchema)]).describe(SCHEDULE_DESCRIPTION);
...
const OPTIONAL_ID_ENTRY_TYPES = /* @__PURE__ */ new Set([
	"parallel",
	"conditional",
	"foreach",
	"loop"
...
function normalizeEntry(entry) {
	...
	if (typeof normalized.type === "string" && OPTIONAL_ID_ENTRY_TYPES.has(normalized.type)) dropNullKeys(normalized, ["id"]);
...
Object.defineProperty(exports, "workflowBuilderScheduleConfigSchema", {
	enumerable: true,
	get: function() {
		return workflowBuilderScheduleConfigSchema;
	}
});
Object.defineProperty(exports, "workflowBuilderScheduleSchema", {
	enumerable: true,
	get: function() {
		return workflowBuilderScheduleSchema;
	}
});
Object.defineProperty(exports, "workflowBuilderSleepEntryInputSchema", {
	enumerable: true,
	get: function() {
		return workflowBuilderSleepEntryInputSchema;
	}
});
...
Object.defineProperty(exports, "workflowBuilderSleepUntilEntryInputSchema", {
	enumerable: true,
	get: function() {
		return workflowBuilderSleepUntilEntryInputSchema;
	}
});
...
//# sourceMappingURL=builder-IMBNoGnF.cjs.map