...
const require_task_state_processor = require("./task-state-processor-6NDCaFdG.cjs");
...
*/
const RUN_REGISTRY_TTL_MS = 7200 * 1e3;
/**
...
* harmless.
*/
const RUN_REGISTRY_HEARTBEAT_MS = 60 * 1e3;
...
function runActivityHeartbeat() {
	for (const runId of activeRuns.keys()) globalRunRegistry.get(runId);
}
/**
...
function serializeScorersConfig(scorers) {
	...
	for (const [key, entry] of Object.entries(scorers)) {
		...
		if (entry.filter) scorerEntry.filter = entry.filter;
...
async function resolveRuntimeDependencies(options) {
	...
	let tools = globalEntry?.baseTools ?? globalEntry?.tools ?? {};
...
async function prepareForDurableExecution(options) {
	...
	if (inputProcessors.length > 0) try {
		const { ProcessorRunner } = await Promise.resolve().then(() => require("./trip-wire-w1gl8It7.cjs")).then((n) => n.runner_exports);
	...
	const backgroundTaskManager = execOptions?.disableBackgroundTasks || execOptions?.backgroundTaskPolicy?.allowToolDispatch === false ? void 0 : mastra?.backgroundTaskManager;
	let tools = {};
	try {
	...
	if (!tripwireData) {
		await require_agent.fireClientToolOutputHooks({
			messages,
			tools,
			abortSignal: execOptions?.abortSignal,
			logger
		});
		await require_agent.applyClientToolModelOutput({
			messageList,
			tools,
			logger
		});
		...
		execute: async (params) => {
			...
			for (let modelIndex = 0; modelIndex < modelList.length; modelIndex++) {
				...
				for (let attempt = 0; attempt <= maxRetries; attempt++) {
					...
					terminalAttemptContext = { recordTerminalError: (error) => {
						require_agent.recordTerminalErrorMessage({
							messageList,
					...
					const textDeltas = [];
					let materializeStreamedMessages;
					try {
						...
						if (stepInputProcessors.length || require_trip_wire.isMaybeAnthropicWithoutAssistantPrefill(currentModel)) {
							...
							try {
								...
								if (processInputStepResult.tools) {
									...
									}, void 0, execOptions.autoResumeSuspendedTools, Boolean(registryEntry?.backgroundTaskManager));
								...
								} else if (registryEntry?.baseTools) registryEntry.tools = registryEntry.baseTools;
							} catch (error) {
								if (error instanceof require_trip_wire.TripWire) {
						...
						if (cachedResponse) {
							...
							try {
								...
								modelResult = require_agent.execute({
									runId,
									model: currentModel,
									providerOptions: currentProviderOptions,
									inputMessages,
						...
						try {
							...
							if (executionAbortSignal?.aborted === true || errorObj.name === "AbortError") {
								materializeStreamedMessages();
								return {
									messageListState: messageList.serialize(),
						...
						if (streamError) {
							...
							if (executionAbortSignal?.aborted === true || streamErrorObj.name === "AbortError") {
								materializeStreamedMessages();
								return {
									messageListState: messageList.serialize(),
									text: textDeltas.join(""),
						...
						if ((globalRunRegistry.get(runId)?.abortSignal ?? abortSignal)?.aborted === true || lastError.name === "AbortError") {
							materializeStreamedMessages?.();
							return {
								messageListState: messageList.serialize(),
								text: textDeltas.join(""),
...
*/
async function processChunkThroughOutputProcessors(chunk, registryEntry, pubsub, runId, agentName, logger, messageList, observabilityContext) {
	...
	try {
		const { part: processed, blocked, reason, tripwireOptions, processorId } = await runner.processPart(chunk, registryEntry.processorStates, observabilityContext, registryEntry.requestContext, messageList, 0, pubsub ? { custom: async (data) => {
			await emitChunkEvent(pubsub, runId, data);
		} } : void 0);
		if (blocked) {
			if (pubsub) await emitChunkEvent(pubsub, runId, {
		...
		runner.endStreamProcessorSpans(registryEntry.processorStates);
	}
}
/**
...
function createDurableToolCallStep() {
	return require_agent.createStep$1({
		...
		execute: async (params) => {
			...
			const processorAgentSpan = registryEntry?.resumeAgentSpan ?? registryEntry?.agentSpan ?? (processorAgentSpanData && observability ? observability.rebuildSpan(processorAgentSpanData) : void 0);
			const processorObservabilityContext = processorAgentSpan ? require_observability.createObservabilityContext({ currentSpan: processorAgentSpan }) : void 0;
			...
			const approvalRequestContext = registryEntry?.requestContext ?? restoreRequestContext$1(initData.requestContextEntries, requestContext);
			const requiresApproval = await toolRequiresApproval(tool, effectiveRequireToolApproval, args, {
				toolName,
				requestContext: Object.fromEntries([...approvalRequestContext.entries()].filter(([key]) => key !== "__mastra_requireToolApproval")),
				workspace
			});
				...
				if (!approvalDecision.approved) {
					...
					if (pubsub) try {
						const processed = await processChunkThroughOutputProcessors(await applyToolPayloadTransformToChunk({
							...
							logger
						}), registryEntry, pubsub, runId, initData.agentId, logger, messageList, processorObservabilityContext);
...
function createDurableLLMMappingStep() {
	return require_agent.createStep$1({
		...
		execute: async (params) => {
			...
			const isPendingClientCall = (toolResult) => toolResult.result === void 0 && !toolResult.error && !toolResult.providerExecuted && !isDeniedApproval(toolResult);
			...
			if (toolResults.length > 0) for (const toolResult of toolResults) {
				...
				if (isPendingClientCall(toolResult)) continue;
			...
			const isContinued = toolResults.some(isPendingClientCall) ? false : hasToolErrors ? true : llmOutput.stepResult.isContinued;
...
function rebindRecoveredModelList(persisted, enabledLive) {
	const pool = [...enabledLive];
	const bound = persisted.map((entry) => {
		const i = pool.findIndex((live) => live.id === entry.id);
		return i === -1 ? void 0 : pool.splice(i, 1)[0];
	});
	const boundById = bound.filter(Boolean).length;
	if (persisted.length - boundById === pool.length) for (let i = 0; i < bound.length; i++) bound[i] ??= pool.shift();
	const modelList = [];
	persisted.forEach((entry, i) => {
		const live = bound[i];
		if (live) modelList.push({
			id: entry.id,
			model: live.model,
	...
	return {
		modelList: modelList.length ? modelList : void 0,
		boundById,
...
var DurableAgent = class extends require_agent.Agent {
	...
	*/
	constructor(config) {
		const { agent, id: idOverride, name: nameOverride, pubsub, cache, maxSteps, cleanupTimeoutMs, shouldCache } = config;
	...
	*/
	...
	async #rehydrateRecoveryState({ runId, workflowInput, abortController, recoveryLease }) {
		...
		if (threadId && resourceId) requestContext.set("MastraMemory", {
			thread: { id: threadId },
			resourceId,
		...
		if (persistedModelList?.length) try {
			const enabledLive = (await wrapped.getModelList(requestContext) ?? []).filter((entry) => entry.enabled !== false);
			const rebound = rebindRecoveredModelList(persistedModelList, enabledLive);
			modelList = rebound.modelList;
			if (rebound.unbound > 0) this.#mastra?.getLogger?.()?.warn?.(`[DurableAgent] recover(${runId}) model list drifted (persisted ${persistedModelList.length} enabled entries, resolved ${enabledLive.length}); bound ${rebound.boundById} by id, ${rebound.boundByPosition} by position; ${rebound.unbound} will resolve from serialized config`);
		} catch (error) {
			this.#mastra?.getLogger?.()?.warn?.(`[DurableAgent] Failed to resolve model list during recover(${runId}): ${error}`);
		}
		recoveryLease.assertOwned();
		let memory;
		try {
		...
		return {
			...
			registryEntry: {
				...
				errorProcessors,
				processorStates,
				drainPendingSignals: (scope) => wrapped.__getDrainPendingSignals()(runId, scope),
				cleanup: () => {}
			}
		};
	}
	...
	*/
		...
		if (this.#cacheConfig === false) {
			...
			if (this.#shouldCache) this.logger.warn(`[DurableAgent:${this.id}] 'shouldCache' is ignored because the configured pubsub is already a CachingPubSub. Pass 'shouldCache' to that CachingPubSub instead.`);
		} else {
			...
			this.#cachingPubsub = new require_topics.CachingPubSub(this.#innerPubsub, resolvedCache, { shouldCache: (topic) => !require_topics.isRunLocalTopic(topic) && (userShouldCache?.(topic) ?? true) });
		}
	}
	/**
	...
	*/
	emitErrorInBackground(runId, error) {
		this.emitError(runId, error).catch((publishError) => {
			this.logger.warn(`Failed to publish error event for run ${runId}`, {
				runId,
				error: publishError
			});
		});
	...
	*/
	abortThreadStream(options) {
		const runId = require_storage.agentThreadStreamRuntime.getActiveThreadRunId(options, this.getPubSub());
	...
	*/
	...
	#isRunExecuting(runId) {
		return this.#runRegistry.get(runId) !== void 0 || globalRunRegistry.get(runId) !== void 0 || require_storage.agentThreadStreamRuntime.hasThreadRun(runId, this.getPubSub());
	}
	/**
	...
	*/
	async stream(messages, options) {
		...
		if (require_storage.agentThreadStreamRuntime.isRunAborted(runId, this.getPubSub())) abortController.abort();
		...
		const { output, cleanup: streamCleanup, ready } = createDurableAgentStream({
			...
			requestContext: registryEntry.requestContext,
			returnScorerData: workflowInput.options.returnScorerData,
			tracingContext: registryEntry.agentSpan ? { currentSpan: registryEntry.agentSpan } : void 0,
			messageList
		});
	...
	*/
	async resume(runId, resumeData, options) {
		...
		if (options?.requestContext) {
			...
			for (const [key, value] of entry.requestContext?.entries() ?? []) if (!resumeRequestContext.has(key)) resumeRequestContext.set(key, value);
			if (entry.requestContext?.has("MastraMemory")) resumeRequestContext.set("MastraMemory", entry.requestContext.get("MastraMemory"));
			else resumeRequestContext.delete("MastraMemory");
		}
		entry.requestContext = resumeRequestContext;
		const globalEntryForContext = globalRunRegistry.get(runId);
		if (globalEntryForContext) globalEntryForContext.requestContext = resumeRequestContext;
		...
		const { output, cleanup: streamCleanup, ready } = createDurableAgentStream({
			...
			tracingContext: resumeSegmentSpan ? { currentSpan: resumeSegmentSpan } : void 0,
	...
	*/
	async listActiveRuns(options = {}) {
		...
		if (!workflowsStore) throw new require_error.MastraError({
			...
			domain: require_error.ErrorDomain.AGENT,
			category: require_error.ErrorCategory.USER,
			text: `DurableAgent "${this.name}" listActiveRuns() requires storage to discover running runs. Register the agent on a Mastra instance with persistent storage (e.g. PostgreSQL, LibSQL). See https://mastra.ai/docs/storage`,
	...
	*/
	async observe(runId, options) {
		...
		const stream = createDurableAgentStream({
			...
			tracingContext: observedAgentSpan ? { currentSpan: observedAgentSpan } : void 0,
...
//# sourceMappingURL=create-durable-agent-CfjlmNSr.cjs.map