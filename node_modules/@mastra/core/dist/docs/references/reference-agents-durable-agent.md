...
Stops a run by ID from any process that shares the agent's [PubSub](https://mastra.ai/docs/server/pubsub). Only the `runId` from `stream()` or `prepare()` is needed, so a later request can stop a run it didn't start. The run ends the same way as `abort()` on the stream result: the `finish` event carries reason `abort`, `onAbort` fires, and `observe()` consumers see the stream end.
...
The abort request is published over PubSub, so a multi-process deployment needs a shared backend such as [`RedisStreamsPubSub`](https://mastra.ai/reference/pubsub/redis-streams). The default `EventEmitterPubSub` only reaches runs in the same process. `InngestAgent` doesn't implement this method. Cancel an Inngest run with the `abort()` returned by its `stream()`.
...
**options.perPage** (`number`): Runs per page. Applies only when page is also set. Must be a positive integer.

**options.page** (`number`): Zero-indexed page number. Applies only when perPage is also set. Must be a non-negative integer.
...
**options.runId** (`string`): Recover a specific run by ID. When set, the discovery filters and pagination are ignored.
...
**runId** (`string`): Unique identifier for this run. Use it later with resume(), observe(), or abortRunStream().
...
**onAbort** (`AgentExecutionOptions['onAbort']`): Called when the run is aborted via abortSignal, result.abort(), abortRunStream(), or abortThreadStream(). Receives the steps completed before the abort and, in text, the assistant text streamed so far.
