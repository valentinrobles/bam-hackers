...
function normalizePerPage(perPageInput, defaultValue) {
	if (perPageInput === false) return Number.MAX_SAFE_INTEGER;
		...
		if (!Number.isInteger(perPageInput) || perPageInput < 0) throw new Error("perPage must be >= 0");
		return perPageInput;
	}
	return defaultValue;
}
...
function calculatePagination(page, perPageInput, normalizedPerPage) {
	if (!Number.isInteger(page) || page < 0) throw new Error("page must be >= 0");
	return {
...
var VersionedStorageDomain = class extends StorageDomain {
	...
	*/
	async getVersions(ids) {
		...
		return (await Promise.all(ids.map((id) => this.getVersion(id)))).filter((version) => version !== null);
	}
	/**
	...
	*/
	async listResolved(args) {
		...
		let resolved;
		if (status === "draft") resolved = await Promise.all(entities.map((entity) => this.resolveEntity(entity, { status })));
		else {
			const activeVersionIds = [...new Set(entities.map((entity) => entity.activeVersionId).filter((id) => !!id))];
			const versions = await this.getVersions(activeVersionIds);
			const versionsById = new Map(versions.map((version) => [version.id, version]));
			resolved = await Promise.all(entities.map(async (entity) => {
				const activeVersion = entity.activeVersionId ? versionsById.get(entity.activeVersionId) : void 0;
				if (activeVersion) return this.mergeVersion(entity, activeVersion);
				if (entity.activeVersionId) this.logger?.warn?.(`Entity ${entity.id} has activeVersionId ${entity.activeVersionId} but version not found. Falling back to latest version.`);
				const latestVersion = await this.getLatestVersion(entity.id);
				return latestVersion ? this.mergeVersion(entity, latestVersion) : entity;
			}));
		}
		return {
			...result,
	...
	*/
	async resolveEntity(entity, options) {
		...
		return version ? this.mergeVersion(entity, version) : entity;
	}
	/**
	* Merges an entity's thin record with a version's snapshot config.
	*/
	mergeVersion(entity, version) {
...
//# sourceMappingURL=filesystem-versioned-4Ajeqjmn.js.map