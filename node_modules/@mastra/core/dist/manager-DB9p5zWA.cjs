...
async function executeTarget(target, targetType, item, options) {
	...
	try {
		...
		switch (targetType) {
			case "agent":
				executionPromise = executeAgent(target, item, signal, options?.requestContext, options?.experimentId, options?.versions, options?.toolMocks, options?.unmockedToolPolicy, (assignedTraceId) => {
...
*/
async function executeAgent(agent, item, signal, requestContext, experimentId, versions, toolMocks, unmockedToolPolicy, onTraceIdAssigned) {
	...
	const assignedTraceId = (0, crypto$1.randomUUID)().replaceAll("-", "");
	...
	try {
		...
		if (mockReport?.failure) return toolMockFailureResult(mockReport, assignedTraceId);
		throw error;
	}
	...
	if (toolMockReport?.failure) return toolMockFailureResult(toolMockReport, traceId ?? assignedTraceId);
	return {
		output: {
			text: result.text,
	...
	try {
		if (config.data) {
			...
			items = versionItems.map((v) => ({
				...
				expectedTrajectory: v.expectedTrajectory ?? void 0,
				requestContext: v.requestContext ?? void 0,
				metadata: v.metadata ?? void 0,
				toolMocks: v.toolMocks ?? void 0,
				unmockedToolPolicy: v.unmockedToolPolicy ?? void 0,
				scorerIds: v.scorerIds ?? void 0
			}));
	...
	try {
		...
		await pMap(items.map((item, idx) => ({
			...
			try {
				...
				const metadataSnapshot = item.metadata === void 0 ? void 0 : structuredClone(item.metadata);
				const cloneMetadataSnapshot = () => metadataSnapshot === void 0 ? void 0 : structuredClone(metadataSnapshot);
				const itemWithMetadataSnapshot = () => ({
					...item,
				...
				if (!preflightError) {
					...
					const flatScores = await runScorersForItem(itemScorers, itemWithMetadataSnapshot(), execResult.output, storage ?? null, experimentId, targetType ?? "agent", targetId ?? "inline", item.id, execResult.scorerInput, execResult.scorerOutput, execResult.traceId ?? void 0, workflowData, persistScores);
					const stepScores = await runStepScorersForItem(itemStepScorers, itemWithMetadataSnapshot(), workflowData, storage ?? null, experimentId, targetType ?? "agent", targetId ?? "inline", item.id, execResult.traceId ?? void 0, persistScores);
...
//#endregion
//#region src/datasets/experiment-traces.ts
...
*/
const UNSUPPORTED_CASCADE_ERROR_IDS = /* @__PURE__ */ new Set(["OBSERVABILITY_STORAGE_BATCH_DELETE_TRACES_NOT_IMPLEMENTED", "OBSERVABILITY_STORAGE_BATCH_DELETE_TRACES_SCOPE_NOT_SUPPORTED"]);
function isUnsupportedCascadeError(error) {
	const id = error?.id;
...
async function collectExperimentTraceIds(args) {
	...
	for (;;) {
		const { results, pagination } = await args.experimentsStore.listExperimentResults({
			...
			pagination: {
				page,
				perPage: TRACE_ID_COLLECTION_PAGE_SIZE
			}
		});
		for (const result of results) if (result.traceId) traceIds.add(result.traceId);
...
*/
async function deleteExperimentTraces(args) {
	...
	if (!observabilityStore) {
		args.logger?.warn(`Skipping trace deletion for experiment ${args.experimentId}: storage has no observability domain. ${traceIds.length} trace(s) were left in place.`);
		return [];
	}
	const organizationId = args.filters?.organizationId;
	for (let i = 0; i < traceIds.length; i += require_constants.BATCH_DELETE_TRACES_MAX_IDS) try {
		await observabilityStore.batchDeleteTraces({
			traceIds: traceIds.slice(i, i + require_constants.BATCH_DELETE_TRACES_MAX_IDS),
			...organizationId !== void 0 ? { organizationId } : {}
		});
	} catch (error) {
		if (!isUnsupportedCascadeError(error) || i > 0) throw error;
		args.logger?.warn(`Skipping trace deletion for experiment ${args.experimentId}: ${error.message}. ${traceIds.length} trace(s) were left in place.`);
		return [];
	}
	return traceIds;
}
...
var Dataset = class {
	...
	*/
	async purgeItem(args) {
		return (await this.#getDatasetsStore()).purgeItem({
			id: args.itemId,
			datasetId: this.id,
			filters: this.#scope
		});
	...
	*/
	async updateExperiment(args) {
		...
		return (await this.#getExperimentsStore()).updateExperiment({
			id: args.experimentId,
	...
	async runExperimentItem(args) {
		...
		return executeExperimentItem({
			...
			item: {
				...
				input: item.input,
				groundTruth: item.groundTruth,
				expectedTrajectory: item.expectedTrajectory ?? void 0,
				requestContext: item.requestContext ?? void 0,
				metadata: item.metadata ?? void 0,
				scorerIds: item.scorerIds ?? void 0
			},
	...
	*/
	async deleteExperiment(args) {
		...
		const experimentsStore = await this.#getExperimentsStore();
		const storage = this.#mastra.getStorage();
		if (storage) await deleteExperimentTraces({
			storage,
			experimentsStore,
			experimentId: args.experimentId,
			...this.#scope !== void 0 ? { filters: this.#scope } : {},
			logger: this.#mastra.getLogger()
		});
		return experimentsStore.deleteExperiment({
			id: args.experimentId,
			filters: this.#scope
		});
	}
...
//# sourceMappingURL=manager-DB9p5zWA.cjs.map