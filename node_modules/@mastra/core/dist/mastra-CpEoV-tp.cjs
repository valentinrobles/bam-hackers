...
const require_background_tasks = require("./background-tasks-niWVHMFb.cjs");
const require_utils$1 = require("./utils-CyQ84CkA.cjs");
...
const require_schedules = require("./schedules-BePwBBKJ.cjs");
...
const require_storage = require("./storage-B9R3bmN-.cjs");
const require_source = require("./source-CVacgqJ6.cjs");
const require_storage$1 = require("./storage-DYAE1FYc.cjs");
const require_create_durable_agent = require("./create-durable-agent-CfjlmNSr.cjs");
const require_manager = require("./manager-DB9p5zWA.cjs");
...
const require_tool_loop_agent = require("./tool-loop-agent-CeOaU0Pc.cjs");
const require_pull_transport = require("./pull-transport-BcT4QFgZ.cjs");
const require_worker = require("./worker-Cfe6Sm3O.cjs");
const require_builder = require("./builder-IMBNoGnF.cjs");
...
*/
function encodeDeclarativeScheduleId(id) {
...
function legacyDeclarativeScheduleRowId(workflowId, scheduleId) {
...
*/
function ownerWorkflowIdForRow(rowId, workflowIds) {
...
function serializableWorkerConfig(config) {
...
* ```
*/
const attachedLoggerOwners = /* @__PURE__ */ new WeakMap();
/**
...
const DEFAULT_SHUTDOWN_DRAIN_TIMEOUT_MS = 5e3;
/**
...
var Mastra = class Mastra {
	...
	#wiredLoggers = /* @__PURE__ */ new WeakSet();
	#fallbackWrappers = /* @__PURE__ */ new WeakMap();
	...
	*/
	#failedDynamicWorkflowIds = /* @__PURE__ */ new Set();
	/**
	...
	*/
	...
	#pushSubscription;
	#inFlightPushEvents = /* @__PURE__ */ new Set();
	#activeEventedRuns = /* @__PURE__ */ new Set();
	#userEventSubscriptions = [];
	#schedulerWakeSubscription;
	#events = {};
	...
	static INTERNAL_WORKFLOW_TTL_MS = require_utils$1.readPositiveIntEnv("MASTRA_SUSPENDED_RUN_TTL_MS", 1800 * 1e3);
	...
	getWorkerConfig() {
		...
		return {
			...
			custom: [...runningWorkerNames].filter((name) => ![
				"orchestration",
				"scheduler",
				"backgroundTasks"
			].includes(name)).sort()
	...
	*/
	registerExporter(exporter, instance, entrypoint) {
		if (this.#observability instanceof require_observability.NoOpObservability) {
			...
			this.#observability.setLogger({ logger: this.#observabilitySafeLogger() });
	...
	*/
	constructor(config) {
		...
		this.#environment = config?.environment ?? (process.env.MASTRA_DEV === "true" ? "development" : process.env.NODE_ENV);
		...
		if (config?.storage) {
			...
			queueMicrotask(() => {
				...
				this.#logger?.warn("No `storage` configured on Mastra — falling back to an in-memory store. In-memory storage is not durable: all data is lost on restart, and it is not safe for production. Configure a persistent storage adapter (e.g. @mastra/libsql, @mastra/pg, @mastra/cloudflare). See https://mastra.ai/docs/storage");
			});
		}
	...
	#collectDeclarativeSchedules() {
		...
		for (const workflow of Object.values(workflows ?? {})) {
			...
			for (const cfg of configs) {
				...
				const legacyScheduleId = isArrayForm ? legacyDeclarativeScheduleRowId(workflow.id, cfg.id) : legacyDeclarativeScheduleRowId(workflow.id);
				out.push({
					scheduleId,
					legacyScheduleId,
	...
	*/
	async registerDeclarativeSchedules(schedulesStore) {
		...
		const declared = this.#collectDeclarativeSchedules().map((entry) => {
			...
			if (entry.legacyScheduleId !== entry.scheduleId && legacyRow?.target.type === "workflow" && legacyRow.target.workflowId === entry.workflowId) return {
				...entry,
				scheduleId: entry.legacyScheduleId
			};
			return entry;
		});
		const declaredIds = new Set(declared.map((d) => d.scheduleId));
		...
		for (const { scheduleId, workflowId, cfg } of declared) try {
			...
			const definitionHash = require_workflow_event_processor.computeScheduleDefinitionHash(workflowsById.get(workflowId)?.serializedStepGraph);
		...
		for (const row of allRows) {
			...
			const targetWorkflowId = row.target.type === "workflow" ? row.target.workflowId : void 0;
			const failedOwnerId = targetWorkflowId && this.#failedDynamicWorkflowIds.has(targetWorkflowId) ? targetWorkflowId : ownerWorkflowIdForRow(row.id, this.#failedDynamicWorkflowIds);
			if (failedOwnerId !== void 0) {
				this.#logger?.warn?.(`Keeping declarative schedule "${row.id}": dynamic workflow "${failedOwnerId}" failed to load this boot.`);
				continue;
			}
			const ownerWorkflowId = targetWorkflowId ?? ownerWorkflowIdForRow(row.id, declaredIdsByWorkflow.keys()) ?? ownerWorkflowIdFromRowId(row.id);
	...
	*/
	async resolveVersionedAgent(agent, version) {
		...
		if (!editor && !("versionId" in version)) return agent;
		if (!editor) {
			const error = new require_error.MastraError({
	...
	addAgent(agent, key, options) {
		...
		if (require_agent.isDurableAgentLike(agent)) {
			...
			for (const workflow of durableWorkflows) {
				this.addWorkflow(workflow, workflow.id);
				if (this.#workflows[workflow.id] === workflow) this.#hiddenWorkflowKeys.add(workflow.id);
			}
	...
	*/
	__registerFsAgents(fsAgents) {
		...
		if (registeredCount > 0 && !this.#fsAgentUsageTracked) {
			...
			require_feature_telemetry.trackFeatureUsage("file_based_agents", { fs_agent_count: registeredCount });
		}
	}
	/**
	...
	* @internal
	*/
	__hasLocalWorkflowExecution() {
		if (this.#pushSubscription) return true;
		return !!this.#workers.find((w) => w.name === "orchestration")?.isRunning;
	}
	...
	*/
	...
	#replaceDynamicWorkflow(workflow, key) {
		...
		this.#hiddenWorkflowKeys.delete(key);
		this.registerStaticWorkflowScorers(workflow);
		if (collectWorkflowScheduleConfigs(workflow).length > 0) this.#hasScheduledWorkflow = true;
	}
	/**
	...
	*/
	async addDynamicWorkflows(defs) {
		...
		if (this.#findSchedulerWorker()?.scheduler) try {
			const schedulesStore = await this.#storage?.getStore("schedules");
			if (schedulesStore) await this.registerDeclarativeSchedules(schedulesStore);
		} catch (error) {
	...
	* @internal
	*/
	async __publishSchedulerWake(scheduleId) {
		try {
			await this.#pubsub.publish(SCHEDULER_WAKE_TOPIC, {
				type: SCHEDULER_WAKE_EVENT,
				runId: scheduleId,
				data: {}
			});
		} catch (err) {
			this.#logger?.warn?.("Failed to publish scheduler wake event", err);
		}
	}
	/** Subscribe until this process's scheduling workers start. Idempotent. */
	async #wireSchedulerWakeSubscription() {
		if (this.#schedulerWakeSubscription) return;
		const cb = async (event, ack, nack) => {
			try {
				...
				await nack?.();
				this.#logger?.warn?.("Failed to start scheduling workers from wake event", err);
			}
		};
		...
		try {
			...
			if (this.#schedulerWakeSubscription === cb) this.#schedulerWakeSubscription = void 0;
			throw err;
		}
	}
	async #unwireSchedulerWakeSubscription() {
		const cb = this.#schedulerWakeSubscription;
		if (!cb) return;
		this.#schedulerWakeSubscription = void 0;
	...
	*/
	async #detectPersistedSchedulerWork() {
		...
		try {
			...
			if (rows.some((row) => !dispatchDisabled || row.id !== "__mastra_notification_dispatch")) this.#schedulerRequested = true;
		} catch (err) {
	...
	*/
	...
	async #startSchedulingWorkers() {
		...
		if (!schedulerWorker) {
			schedulerWorker = new require_worker.SchedulerWorker(this.#schedulerConfig);
		...
		if (!this.#workerFilter || this.#workerFilter.has("agent-schedule")) {
			let agentScheduleWorker = this.#findAgentScheduleWorker();
			if (!agentScheduleWorker) {
				const { AgentScheduleWorker } = await Promise.resolve().then(() => require("./worker-CrUyhf1g.cjs"));
	...
	*/
	#wireLoggerObservability(logger) {
		const inner = logger;
		if (require_logger.isAdaptableLogger(inner)) {
			...
			if (previousOwner && previousOwner !== this && this.#loggerAdapterOptions.export) try {
				inner.warn("This logger instance is already wired to another Mastra instance; re-attaching. Its observability export now targets the newest Mastra. Create a separate logger per Mastra instance to keep exports isolated.");
			} catch {}
			attachedLoggerOwners.set(ownershipKey, this);
			inner.__attachObservability({
				resolveTraceFields: require_logger_index.resolveTraceFields,
				getLogSink: () => {
					if (!this.#loggerAdapterOptions.export || require_logger_index.isObservabilityExportSuppressed()) return void 0;
					const span = require_utils.resolveCurrentSpan();
					return span?.observabilityInstance?.getLoggerContext?.(span) ?? this.#observability.getDefaultInstance()?.getLoggerContext?.();
				},
				options: this.#loggerAdapterOptions
			});
			return logger;
		}
		if (inner instanceof require_logger_index.DualLogger) return this.#wireLoggerObservability(inner.baseLogger);
		const existing = this.#fallbackWrappers.get(inner);
		if (existing) return existing;
		try {
			inner.debug?.("Configured logger does not support observability adapters; falling back to DualLogger (deprecated). Implement __attachObservability() on your logger to get trace-correlated stdout.");
		} catch {}
		const wrapper = new require_logger_index.DualLogger(inner, this.#loggerAdapterOptions.export ? () => this.#observability.getDefaultInstance()?.getLoggerContext?.() : void 0);
	...
	*/
	#observabilitySafeLogger() {
		return require_logger_index.createExportSuppressedLogger(this.#logger instanceof require_logger_index.DualLogger ? this.#logger.baseLogger : this.#logger);
	}
	/**
	...
	*/
	async startWorkers(name) {
		...
		const schedulerSelected = !this.#schedulerDisabled() && (!this.#workerFilter || this.#workerFilter.has("scheduler"));
		const schedulerForcedByFilter = schedulerSelected && (this.#workerFilter?.has("scheduler") ?? false);
		if (!name && schedulerSelected && this.#storage && !this.#findSchedulerWorker()?.isRunning) try {
			await this.#wireSchedulerWakeSubscription();
		} catch (err) {
			this.#logger?.warn?.("Failed to subscribe to scheduler wake events; scheduler will not wake on demand", err);
		}
		if (!name && schedulerSelected && !this.#schedulerRequested && !schedulerForcedByFilter) await this.#detectPersistedSchedulerWork();
		...
		if (!name && this.#schedulerRequested && !this.#findSchedulerWorker()) await this.#ensureSchedulingWorkersStarted();
		if (this.#findSchedulerWorker()?.isRunning) await this.#unwireSchedulerWakeSubscription();
	}
	/**
	...
	async #wirePushWorkflowSubscription() {
		...
		if (modes.includes("push") && !modes.includes("pull") && !this.#pushSubscription) {
			const cb = (event, ack, nack) => {
				...
				const inFlight = this.handleWorkflowEvent(event).then((result) => {
					if (result.ok) {
				...
				inFlight.finally(() => this.#inFlightPushEvents.delete(inFlight));
			};
	...
	* pubsub is flushed.
	*/
	async stopWorkers(options) {
		const drainTimeout = require_pull_transport.assertDrainTimeout(options?.drainTimeout ?? DEFAULT_SHUTDOWN_DRAIN_TIMEOUT_MS, "stopWorkers");
		const deadline = Date.now() + drainTimeout;
		const remaining = () => Math.max(0, deadline - Date.now());
		this.#workersStarted = false;
		while (this.#schedulingWorkersStartPromise) await this.#schedulingWorkersStartPromise.catch(() => {});
		while (this.#executionWorkersStartPromise) await this.#executionWorkersStartPromise.catch(() => {});
		for (const worker of [...this.#workers].reverse()) if (worker.isRunning) await worker.stop({ drainTimeout: remaining() });
		if (this.#pushSubscription) {
			await this.#pubsub.unsubscribe(this.#pushSubscription.topic, this.#pushSubscription.cb);
			this.#pushSubscription = void 0;
		}
		if (this.#inFlightPushEvents.size > 0) await this.#awaitBounded(Promise.allSettled([...this.#inFlightPushEvents]), remaining(), `${this.#inFlightPushEvents.size} in-flight workflow event(s)`);
	...
	*/
	async #awaitBounded(promise, timeoutMs, description) {
		let timeoutHandle;
		try {
			const outcome = await Promise.race([promise.then((value) => ({
				timedOut: false,
				value
			})), new Promise((resolve) => {
	...
	*/
	#syncGatewayRegistry() {
		try {
			...
			Promise.resolve().then(() => require("./provider-registry-BYH6y4zy.cjs")).then((n) => n.provider_registry_exports).then(async ({ GatewayRegistry }) => {
	...
	*/
	async shutdown(options) {
		const drainTimeout = require_pull_transport.assertDrainTimeout(options?.drainTimeout ?? DEFAULT_SHUTDOWN_DRAIN_TIMEOUT_MS, "shutdown");
		const deadline = Date.now() + drainTimeout;
		this.__unregisterHooks();
		const awaited = /* @__PURE__ */ new Set();
		for (;;) {
			const pendingRuns = [...this.#activeEventedRuns, ...require_create_durable_agent.getActiveDurableAgentWorkflowExecutions(this)].filter((run) => !awaited.has(run));
			if (pendingRuns.length === 0) break;
			pendingRuns.forEach((run) => awaited.add(run));
			const drained = await this.#awaitBounded(Promise.allSettled(pendingRuns), Math.max(0, deadline - Date.now()), `${pendingRuns.length} in-flight evented workflow run(s)`);
			if (!drained) break;
			drained.forEach((result) => {
				if (result.status === "rejected") this.#logger?.error("Evented workflow run failed during shutdown", { error: result.reason });
			});
		}
		if (this.#backgroundTaskManager) await this.#backgroundTaskManager.shutdown({ deadline });
		await this.stopWorkers({ drainTimeout: Math.max(0, deadline - Date.now()) });
		const workspaceIds = Object.keys(this.#workspaces);
		(await Promise.allSettled(workspaceIds.map(async (id) => {
			const entry = this.#workspaces[id];
			if (!entry) return;
			await entry.workspace.stop();
			await this.removeWorkspace(id);
		}))).forEach((result, index) => {
			if (result.status === "rejected") this.#logger?.error("Failed to tear down workspace during shutdown", {
...
//# sourceMappingURL=mastra-CpEoV-tp.cjs.map