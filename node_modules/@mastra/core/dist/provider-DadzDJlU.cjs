...
var CompositeAuth = class CompositeAuth extends MastraAuthProvider {
	providers;
	...
	mapAuthenticatedUserToResourceId(user) {
		...
		if (typeof provider?.mapUserToResourceId !== "function") return;
		const resourceId = provider.mapUserToResourceId(user);
		if (resourceId === void 0 && provider instanceof CompositeAuth) return;
		if (typeof resourceId !== "string" || !resourceId.trim()) throw new Error("mapUserToResourceId must return a non-empty string for the authenticated user");
		return resourceId;
	}
	/**
...
//# sourceMappingURL=provider-DadzDJlU.cjs.map