...
async function rehydrateWorkflow(def, mastra, opts) {
	...
	let wf;
	if (def.schedule === void 0) wf = require_agent.createWorkflow(baseParams);
	else try {
		wf = require_agent.createWorkflow({
			...baseParams,
		...
		wf = require_agent.createWorkflow(baseParams);
	}
	...
	switch (entry.type) {
		...
		case "conditional": {
			...
			const serializedConditions = entry.serializedConditions ?? steps.map((s, i) => ({
				...
				fn: require_predicate_index.derivePredicateLabel(predicates[i])
			}));
			const live = {
				type: "conditional",
				...entryOptionFields(entry),
		...
		case "loop": {
			...
			const serializedCondition = entry.serializedCondition ?? {
				...
				fn: require_predicate_index.derivePredicateLabel(predicate)
			};
			const live = {
				type: "loop",
				...entryOptionFields(entry),
...
//#endregion
Object.defineProperty(exports, "entryOptionFields", {
	enumerable: true,
	get: function() {
		return entryOptionFields;
	}
});
...
Object.defineProperty(exports, "toStorableGraph", {
	enumerable: true,
	get: function() {
		return toStorableGraph;
	}
});

//# sourceMappingURL=rehydrate-Bga9FySw.cjs.map