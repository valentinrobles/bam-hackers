...
var Scheduler = class extends MastraBase {
	...
	*/
	#staleDefinitionCounts = /* @__PURE__ */ new Map();
	...
	*/
		...
		const next = prev + 1;
		this.#staleDefinitionCounts.set(schedule.id, {
			fireAt: schedule.nextFireAt,
			count: next
		});
		const targetSummary = schedule.target.type === "workflow" ? { workflowId: schedule.target.workflowId } : { agentId: schedule.target.agentId };
		const limit = this.#config.staleSkipsBeforeEscalation ?? DEFAULT_STALE_SKIPS_BEFORE_ESCALATION;
		if (prev === 0) this.logger.warn("Local target definition differs from the schedule row; leaving fire for an instance running the current build", {
			scheduleId: schedule.id,
			targetType: schedule.target.type,
			...targetSummary
		});
		else if (next === limit) {
			this.logger.error("Schedule has been skipped repeatedly because no local target definition matches the schedule row; it will not fire until an instance running the recorded build is available", {
				scheduleId: schedule.id,
				targetType: schedule.target.type,
				...targetSummary,
				consecutiveStaleSkips: next
			});
			this.#recordStaleSkip(schedule, next);
		}
		return false;
	}
	/**
	...
	*/
	async #recordStaleSkip(schedule, consecutiveStaleSkips) {
		try {
			...
			this.logger.warn("Failed to record stale-definition skip", {
				scheduleId: schedule.id,
				error: err
			});
		}
	...
	async #publishTargetStart(schedule, claimId) {
		switch (schedule.target.type) {
			case "workflow": {
				...
				}, localOnly ? { localOnly: true } : void 0);
				return;
			}
			case "agent":
...
//# sourceMappingURL=scheduler-3bLZ3ugK.js.map