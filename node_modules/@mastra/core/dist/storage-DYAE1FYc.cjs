...
var AgentThreadStreamRuntime = class {
	...
	*/
	...
	async claimThreadOwnership(agent, options, pubsub) {
		...
		const onEvent = async (event) => {
			...
			try {
				const accepted = await this.#startClaimedIdleRun(state, resolvedPubSub, key, owner, data.runId, require_signals.createSignal(data.signal), Date.now() + data.timeoutMs, () => active && state.claimedThreadOwners.get(key)?.unsubscribe === unsubscribe);
	...
	async discoverThreadPeers(options = {}, pubsub) {
		const resolvedPubSub = this.#getPubSub(pubsub);
		const state = this.#getState(resolvedPubSub);
		const requestId = (0, crypto.randomUUID)();
	...
	async #startClaimedIdleRun(state, pubsub, key, owner, runId, signal, expiresAt, isOwnerActive) {
		...
		try {
			...
			const message = require_error.getErrorFromUnknown(error).message;
			state.threadKeysByRunId.delete(runId);
	...
	async #deliverToClaimedThreadOwner(pubsub, key, runId, signal, targetSourceId) {
		...
		return new Promise((resolve, reject) => {
			...
			})).catch((error) => finish({ error: require_error.getErrorFromUnknown(error) }));
		});
	}
	...
	prepareRunOptions(options, pubsub) {
		...
		if (requestContext && typeof controllerContext === "object" && controllerContext !== null) {
			const preparedRequestContext = new require_request_context.RequestContext(requestContext.entries());
	...
	*/
					...
					while (!done || pendingRuns.length > 0) {
						...
						try {
							while (true) {
								...
								if (!require_signal_exclusions.isSignalChunkExcluded(partWithRunId, options.hideSignals)) yield partWithRunId;
								if (done) break;
	...
	async sendStateSignal(agent, stateInput, target, pubsub) {
		...
		const applied = await require_signal_exclusions.applyStateSignal({
			input: stateInput,
			memory,
			thread: {
				...loadedThread,
		...
		if (runId) {
			...
			if (key && state.activeThreadRunIds.get(key) === runId) {
				...
				if (claimedOwnerDiscovery) {
					const accepted = this.#deliverAfterClaimedOwnerDiscovery(this.#getPubSub(pubsub), key, (0, crypto.randomUUID)(), signal, claimedOwnerDiscovery);
					accepted.catch(() => {});
					return {
						signal,
						accepted
...
//# sourceMappingURL=storage-DYAE1FYc.cjs.map