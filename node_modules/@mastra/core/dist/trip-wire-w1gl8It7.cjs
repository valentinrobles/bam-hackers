...
const require_observability = require("./observability-BFBzlgy3.cjs");
...
const require_types = require("./types-CbgLPoEt.cjs");
...
const require_signal_exclusions = require("./signal-exclusions-CPASpWf2.cjs");
const require_llm = require("./llm-DTp5hqfJ.cjs");
	...
	try {
		...
		const result = await agent.stream(prompt, {
			...
			structuredOutput: {
				...
				jsonPromptInjection: fallbackJsonPromptInjection ?? (streamOptions.structuredOutput.jsonPromptInjection === "inline" || streamOptions.structuredOutput.jsonPromptInjection === "system" ? streamOptions.structuredOutput.jsonPromptInjection : true)
			}
		});
		onStream?.(result);
		try {
...
//#endregion
//#region src/processors/provider-history-compat.ts
function getErrorCandidates(error) {
	const candidates = [error.message];
	if (require_dist.APICallError.isInstance(error) && typeof error.responseBody === "string") candidates.push(error.responseBody);
...
function matchesRule(error, rule) {
	...
	if (require_dist.APICallError.isInstance(error)) return getErrorCandidates(error).some(matches);
	if (error instanceof Error) return getErrorCandidates(error).some(matches);
	return false;
}
...
function buildToolIdMap(messages) {
	...
	for (const msg of messages) {
		...
		for (const part of msg.content.parts) if (part.type === "tool-invocation") {
			const id = part.toolInvocation.toolCallId;
			if (id && !VALID_TOOL_ID_PATTERN.test(id) && !idMap.has(id)) idMap.set(id, sanitizeToolId(id));
		}
		if (msg.content.toolInvocations) for (const inv of msg.content.toolInvocations) {
...
function rewriteToolIds(messages, idMap) {
	for (const msg of messages) {
		if (msg.content?.parts) for (let i = 0; i < msg.content.parts.length; i++) {
			const part = msg.content.parts[i];
			if (part.type === "tool-invocation") {
				const oldId = part.toolInvocation.toolCallId;
				const newId = idMap.get(oldId);
				if (newId) part.toolInvocation = {
					...part.toolInvocation,
					toolCallId: newId
				};
			}
...
function matchesProviderPrefix(model, providerPrefix) {
	...
	if (Array.isArray(model)) return model.some((m) => matchesProviderPrefix(m.model ?? m, providerPrefix));
	const gatewayPattern = new RegExp(`^${providerPrefix}[/:]`, "i");
	const providerPattern = new RegExp(`^${providerPrefix}($|[.\\-])`, "i");
	if (typeof model === "string") return gatewayPattern.test(model);
...
function getModelProviderId(model) {
	if (model == null || typeof model === "function" || typeof model === "string") return void 0;
	if (Array.isArray(model)) {
		for (const entry of model) {
			const provider = getModelProviderId(entry.model ?? entry);
...
function getModelProviderFamily(model) {
...
function getPartProviderFamily(part) {
	if (!part.providerOptions || typeof part.providerOptions !== "object") return void 0;
...
*/
const stripForeignProviderExecutedTools = {
	name: "strip-foreign-provider-executed-tools",
	applyToPrompt({ prompt, model }) {
		const destinationProvider = getModelProviderFamily(model);
		if (!destinationProvider) return void 0;
		const foreignToolCallIds = /* @__PURE__ */ new Set();
		for (const message of prompt) {
			...
			for (const part of message.content) {
				...
				if (sourceProvider && sourceProvider !== destinationProvider) foreignToolCallIds.add(part.toolCallId);
			}
		}
		if (foreignToolCallIds.size === 0) return void 0;
		const rewritten = [];
		for (const message of prompt) {
			if (message.role === "assistant") {
				const content = message.content.filter((part) => part.type !== "tool-call" || !foreignToolCallIds.has(part.toolCallId));
				if (content.length > 0) rewritten.push({
					...message,
					content
				});
				continue;
...
function supportsAssistantPrefill(modelId) {
...
*/
function isMaybeAnthropicWithoutAssistantPrefill(model) {
	if (typeof model === "function") return true;
	if (Array.isArray(model)) return model.some((entry) => isMaybeAnthropicWithoutAssistantPrefill(entry.model ?? entry));
	if (!isMaybeAnthropic(model)) return false;
	const modelId = typeof model === "string" ? model : model && typeof model === "object" && typeof model.modelId === "string" ? model.modelId : void 0;
	if (!modelId) return true;
	return supportsAssistantPrefill(modelId) !== true;
}
function isMaybeAzure(model) {
	if (Array.isArray(model)) return model.some((entry) => isMaybeAzure(entry.model ?? entry));
	if (model && typeof model === "object") {
		const { provider, modelId } = model;
		if (typeof provider === "string" && /^(?:azure|azure-openai)(?:\.[a-z0-9_-]+)?$/i.test(provider)) return true;
		return typeof modelId === "string" && (matchesProviderPrefix(modelId, "azure") || matchesProviderPrefix(modelId, "azure-openai"));
	}
...
*/
function getProtectedAssistantIndex(prompt) {
...
* modified").
*/
function stripReasoningFromPrompt(prompt, shouldStrip = () => true, skipIndex = -1) {
	let mutated = false;
...
function getProtectedAnthropicAssistantIndex(prompt) {
...
function getProviderMetadataForProvider(metadata, provider) {
...
function hasAnthropicSignatureWithoutText(part) {
	const anthropic = getProviderMetadataForProvider(part.providerOptions, "anthropic") ?? getProviderMetadataForProvider(part.providerMetadata, "anthropic");
	return typeof anthropic?.signature === "string" && anthropic.signature.length > 0 && (typeof part.text !== "string" || part.text.length === 0);
}
/**
...
*/
const cerebrasStripReasoningContent = {
...
* boundary.
*/
const anthropicStripEmptySignedReasoningContent = {
...
*/
const anthropicStripForeignReasoningContent = {
	name: "anthropic-strip-foreign-reasoning-content",
	applyToPrompt({ prompt, model }) {
		if (!isMaybeAnthropic(model)) return void 0;
		return stripReasoningFromPrompt(prompt, (part) => !isAnthropicReasoningPart(part), getProtectedAnthropicAssistantIndex(prompt));
	}
};
/**
	...
	applyToPrompt({ prompt, model, messageList }) {
		...
		for (const dbMessage of messageList.get.all.db()) {
			...
			for (const part of dbMessage.content.parts ?? []) {
				...
				for (const value of [anthropic?.signature, anthropic?.redactedData]) if (typeof value === "string" && value && !foreign.has(value)) foreign.set(value, origin);
			}
		}
		if (foreign.size === 0) return void 0;
		let dropped = 0;
		const next = [];
		for (const message of prompt) {
			...
			if (content.length > 0) next.push({
				...message,
				content
			});
		}
		return dropped > 0 ? next : void 0;
	}
};
const SYSTEM_REMINDER_OPEN_TAG = /<system-reminder(?=\s|\/?>)([^>]*)>/g;
const SYSTEM_REMINDER_CLOSE_TAG = /<\/system-reminder>/g;
function rewriteSystemReminderTags(text) {
	return text.replace(SYSTEM_REMINDER_OPEN_TAG, "<memory-context$1>").replace(SYSTEM_REMINDER_CLOSE_TAG, "</memory-context>");
}
/**
* All built-in compat rules. Extend by passing additional rules to the
* `ProviderHistoryCompat` constructor.
*/
const DEFAULT_COMPAT_RULES = [
	...
	anthropicStripForeignSignedReasoning,
	{
		...
		applyToPrompt({ prompt, model }) {
			if (!isMaybeAzure(model)) return void 0;
			let mutated = false;
...
var ProviderHistoryCompat = class {
	...
	processLLMRequest({ prompt, model, messageList }) {
		let current = prompt;
		let mutated = false;
		for (const rule of this.rules) {
			if (!rule.applyToPrompt) continue;
			const next = rule.applyToPrompt({
				prompt: current,
				model,
				messageList
			});
	...
	async processAPIError({ error, messageList, retryCount }) {
		if (retryCount > 0) return;
		const messages = messageList.get.all.db();
		for (const rule of this.rules) {
			if (!rule.fix) continue;
			if (matchesRule(error, rule)) {
				if (rule.fix(messages)) return { retry: true };
			}
		}
	}
};
...
//#endregion
//#region src/processors/span-declaration.ts
/**
...
*/
const WORKFLOW_PHASE_TO_SPAN_PHASE = {
	input: "input",
	inputStep: "inputStep",
...
function toProcessorSpanPhase(phase) {
...
*/
function resolveProcessorSpanName(processor, phase, fallback) {
...
function resolveProcessorSpanAttributes(processor, phase) {
...
var runner_exports = /* @__PURE__ */ require_rolldown_runtime.__exportAll({
	OUTPUT_STREAM_HOOK_DURATION_KEY_PREFIX: () => OUTPUT_STREAM_HOOK_DURATION_KEY_PREFIX,
	...
	*/
	...
	constructor(options) {
		...
		this.span = parentSpan?.createChildSpan({
			type: processor?.spanType ?? "processor_run",
			name: processor ? resolveProcessorSpanName(processor, "output", `output stream processor: ${options.processorName}`) : `output stream processor: ${options.processorName}`,
...
*/
const OUTPUT_STREAM_HOOK_DURATION_KEY_PREFIX = "__outputStreamHookDurationMs_";
...
var ProcessorRunner = class ProcessorRunner {
	...
	*/
	...
	async runComputeStateSignal({ processor, messageList, stepNumber, steps, requestContext, writer, abort, processorState, memory, resourceId, threadId, abortSignal, retryCount, rotateResponseMessageId, observabilityContext }) {
		const computeStateSignal = processor.computeStateSignal?.bind(processor);
		if (!computeStateSignal) return;
		const memoryContext = require_signal_exclusions.parseMemoryRequestContext(requestContext);
		...
		const tracking = require_signal_exclusions.getStateSignalsMetadata(thread.metadata)[stateId];
		const { activeStateSignals, contextWindow, lastSnapshot, deltasSinceSnapshot } = await require_signal_exclusions.resolveStateSignalHistory({
			messageList,
		...
		const result = await computeStateSignal({
			...require_observability.createObservabilityContext({ currentSpan: observabilityContext?.tracingContext?.currentSpan }),
			...
			sendStateSignal: async (stateSignal) => {
				const sendResult = await require_signal_exclusions.applyStateSignal({
					input: stateSignal,
					memory: resolvedMemory,
					thread,
					resourceId: resolvedResourceId,
		...
		if (!result) return;
		await require_signal_exclusions.applyStateSignal({
			input: result,
			memory: resolvedMemory,
			thread,
			resourceId: resolvedResourceId,
		...
		observabilityContext?.tracingContext?.currentSpan?.createEventSpan({
			type: "agent_signal",
			...
			output: {
				...
				hasDelta: Boolean(result.delta)
			}
		});
	}
	async runWorkflowComputeStateSignals({ workflow, messageList, stepNumber, steps, requestContext, writer, memory, resourceId, threadId, abortSignal, retryCount, rotateResponseMessageId, observabilityContext }) {
	...
	*/
	async executeWorkflowAsProcessor(workflow, input, observabilityContext, requestContext, writer, abortSignal) {
		...
		const outputWriter = writer ? (chunk, options) => writer.custom(chunk, options) : void 0;
		if (isPerChunkPhase && workflow.__executeOutputStream) {
			const execute = workflow.__executeOutputStream;
			return require_utils.executeWithContext({
				span: observabilityContext?.tracingContext?.currentSpan,
				fn: () => execute({
					inputData,
					...observabilityContext,
					requestContext: requestContext ?? new require_request_context.RequestContext(),
					outputWriter
				})
			});
		}
		const result = await (await workflow.createRun(isPerChunkPhase ? {
	...
	*/
	async processPart(part, processorStates, observabilityContext, requestContext, messageList, retryCount = 0, writer) {
		...
		try {
			...
			for (const [index, processorOrWorkflow] of this.outputProcessors.entries()) {
				...
				try {
					...
					state?.span?.error({
						error,
						endSpan: true,
						attributes: state.getFinalAttributes()
					});
			...
			if (isFinishChunk) {
				for (const state of processorStates.values()) if (state.span) state.span.end({
	...
	endStreamProcessorSpans(processorStates) {
		for (const state of processorStates.values()) {
			state.span?.end({
				output: state.getFinalOutput(),
				attributes: state.getFinalAttributes()
			});
			for (const [key, value] of Object.entries(state.customState)) if (key.startsWith("__outputStreamSpan_")) {
				const durationKey = OUTPUT_STREAM_HOOK_DURATION_KEY_PREFIX + key.slice(19);
				const hookDurationMs = state.customState[durationKey];
				value?.end(typeof hookDurationMs === "number" ? { attributes: { hookDurationMs } } : void 0);
	...
	*/
	async runProcessInputStep(args) {
		...
		const processors = stepInput.model && isMaybeAnthropicWithoutAssistantPrefill(stepInput.model) ? [...this.inputProcessors, new TrailingAssistantGuard()] : this.inputProcessors;
			...
			try {
				...
				const processMethodArgs = {
					...
					sendStateSignal: async (stateSignal) => {
						...
						const result = await require_signal_exclusions.applyStateSignal({
							input: stateSignal,
							memory: resolvedMemory,
							thread: {
								...loadedThread,
				...
				await this.runComputeStateSignal({
					...
					abort,
					processorState,
					observabilityContext: require_observability.createObservabilityContext({ currentSpan: processorSpan }),
					memory: args.memory,
					resourceId: args.resourceId,
					threadId: args.threadId,
					abortSignal: args.abortSignal,
	...
	*/
	async runProcessLLMRequest(args) {
		const observabilityContext = require_observability.resolveObservabilityContext({ tracingContext: args.tracingContext });
	...
	*/
	...
	static applyMessagesToMessageList(messages, messageList, idsBeforeProcessing, check, defaultSource = "input") {
		...
		for (const message of messages) {
			if (currentById.get(message.id) === message) continue;
			messageList.removeByIds([message.id]);
			if (message.role === "system") {
				const systemText = message.content.content ?? message.content.parts?.map((p) => p.type === "text" ? p.text : "").join("\n") ?? "";
				messageList.addSystem(systemText);
	...
	initialize({ runId, createStream, onResult }) {
		...
		outputStream = new ReadableStream({ async start(controller) {
			try {
				...
				require_types.attachModelStreamTransport(outputStream, require_types.readModelStreamTransport(stream));
...
//#endregion
//#region src/stream/base/step-message-mirrors.ts
const RESPONSE_MESSAGE_IDS = "__responseMessageIds";
function isRecord(value) {
	return !!value && typeof value === "object" && !Array.isArray(value);
}
function withoutMessageMirrors(response) {
	const rest = {};
...
function packStepMessageMirrors(steps) {
	if (!Array.isArray(steps)) return steps;
	return steps.map((step) => {
		if (!isRecord(step) || !isRecord(step.response) || !Array.isArray(step.response.dbMessages)) return step;
		const dbMessages = step.response.dbMessages;
		return {
			...step,
			response: {
...
function unpackStepMessageMirrors(steps, messageList) {
	if (!Array.isArray(steps)) return steps;
	if (!steps.some((step) => isRecord(step) && isRecord(step.response) && RESPONSE_MESSAGE_IDS in step.response)) return steps;
	let messagesById;
	return steps.map((step) => {
		if (!isRecord(step) || !isRecord(step.response) || !(RESPONSE_MESSAGE_IDS in step.response)) return step;
		const messageIds = step.response[RESPONSE_MESSAGE_IDS];
		...
		Object.defineProperties(response, {
			...
			uiMessages: {
				enumerable: true,
				get() {
					return uiMessages ??= require_message_list.convertMessages(getDbMessages()).to("AIV5.UI");
				}
			},
			messages: {
				enumerable: true,
				get() {
					return messages ??= require_message_list.convertMessages(getDbMessages()).to("AIV5.Model");
				}
			}
		});
		return {
...
var MastraModelOutput = class extends require_base.MastraBase {
	...
	*/
	...
	constructor({ model: _model, stream, messageList, options, messageId, initialState }) {
		...
		if (options.logger) this.__setLogger(options.logger);
		...
		if (processorRunner && options.isLLMExecutionStep) {
			...
			const getObservabilityContext = () => observabilityContext ??= require_observability.resolveObservabilityContext(options);
			processedStream = stream.pipeThrough(new stream_web.TransformStream({
				async transform(chunk, controller) {
					...
					if (chunk.type === "finish" && chunk.payload?.stepResult?.reason === "tool-calls" || isDeferredErrorChunk) {
						...
						*/
						...
						const { part: processed, blocked, reason, tripwireOptions, processorId } = await processorRunner.processPart(chunk, processorStates, getObservabilityContext(), options.requestContext, self.messageList, 0, streamWriter);
						const enqueueTripwire = (r, opts, pid) => {
							controller.enqueue({
								type: "tripwire",
								payload: {
						...
						const reprocessed = await processorRunner.drainReprocessParts(processorStates, getObservabilityContext(), options.requestContext, self.messageList, 0, streamWriter);
						for (const r of reprocessed) {
							if (r.blocked) {
								enqueueTripwire(r.reason, r.tripwireOptions, r.processorId);
								return;
					...
					case "finish":
						...
						try {
							if (self.processorRunner && !self.#options.isLLMExecutionStep) {
								...
								if (self.#status === "canceled" && self.#bufferedByStep.text.trim().length > 0) self.messageList.add({
									role: "assistant",
									content: [{
										type: "text",
								...
								self.messageList = await self.processorRunner.runOutputProcessors(self.messageList, require_observability.resolveObservabilityContext(options), self.#options.requestContext, 0, outputResultWriter, outputResult);
								const outputText = resolveOutputTextSkippingCompletionChecks(self.messageList);
								if (self.#status !== "canceled" && lastStep && outputText !== void 0 && outputText !== originalText) lastStep.text = outputText;
	...
	*/
	get fullStream() {
		...
		return stream.pipeThrough(new stream_web.TransformStream({ transform(chunk, controller) {
			if (!require_signal_exclusions.isSignalChunkExcluded(chunk, hideSignals)) controller.enqueue(chunk);
		} }));
	}
	...
	*/
	...
	deserializeState(state) {
		...
		this.#bufferedSteps = unpackStepMessageMirrors(rehydrateStepRequests(state.bufferedSteps, state.bufferedStepRequests), this.messageList);
	}
};
//#endregion
...
Object.defineProperty(exports, "OUTPUT_STREAM_HOOK_DURATION_KEY_PREFIX", {
	enumerable: true,
	get: function() {
		return OUTPUT_STREAM_HOOK_DURATION_KEY_PREFIX;
	}
});
...
Object.defineProperty(exports, "anthropicToolIdFormat", {
	enumerable: true,
	get: function() {
		return anthropicToolIdFormat;
	}
});
Object.defineProperty(exports, "cerebrasStripReasoningContent", {
	enumerable: true,
	get: function() {
		return cerebrasStripReasoningContent;
	}
});
...
Object.defineProperty(exports, "isMaybeAnthropicWithoutAssistantPrefill", {
	enumerable: true,
	get: function() {
		return isMaybeAnthropicWithoutAssistantPrefill;
	}
});
...
Object.defineProperty(exports, "resolveProcessorSpanAttributes", {
	enumerable: true,
	get: function() {
		return resolveProcessorSpanAttributes;
	}
});
Object.defineProperty(exports, "resolveProcessorSpanName", {
	enumerable: true,
	get: function() {
		return resolveProcessorSpanName;
	}
});
...
Object.defineProperty(exports, "toProcessorSpanPhase", {
	enumerable: true,
	get: function() {
		return toProcessorSpanPhase;
	}
});
...
//# sourceMappingURL=trip-wire-w1gl8It7.cjs.map