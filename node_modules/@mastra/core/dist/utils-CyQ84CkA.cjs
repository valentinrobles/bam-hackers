...
function createToolObserve(span) {
	if (!span?.isValid) return require_stream.noopObserve;
	const logger = require_observability.createObservabilityContext({ currentSpan: span }).loggerVNext;
	return {
...
var CoreToolBuilder = class extends require_base.MastraBase {
	...
	constructor(input) {
		...
		const isBackgroundEligible = !!input.backgroundTaskEnabled && require_background_tasks.isToolBackgroundEligible({
		...
		return async (args, execOptions) => {
			...
			if (fgaProvider) {
				...
				await requireFGA({
					...
					permission: require_ee_B4JkfaNy.MastraFGAPermissions.TOOLS_EXECUTE,
					requestContext: toolRequestContext,
					actor: execOptions?.actor,
...
//# sourceMappingURL=utils-CyQ84CkA.cjs.map