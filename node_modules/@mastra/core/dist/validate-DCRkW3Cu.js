...
import { d as validateTemplate, u as collectTemplateStepIds } from "./workflow-event-processor-BSZt3kfu.js";
import { z } from "zod";
...
function validateWorkflowStructure(def) {
	...
	for (const entry of def.graph) if ((entry.type === "sleep" || entry.type === "sleepUntil") && entry.id) seenIds.add(entry.id);
	def.graph.forEach((entry, index) => {
		const path = `graph.${index}`;
		if ((entry.type === "parallel" || entry.type === "conditional" || entry.type === "foreach" || entry.type === "loop") && entry.id !== void 0) if (!entry.id) issues.push({
			code: "missing-step-id",
			path: `${path}.id`,
			message: "Entry id must not be empty when supplied."
		});
		else if (seenIds.has(entry.id)) issues.push({
			code: "duplicate-step-id",
			path: `${path}.id`,
...
//# sourceMappingURL=validate-DCRkW3Cu.js.map