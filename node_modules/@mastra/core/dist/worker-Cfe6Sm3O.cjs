...
var SchedulerWorker = class extends require_pull_transport.MastraWorker {
	...
	async init(deps) {
		...
		const isTargetCurrent = mastra ? (target) => {
			...
			try {
				const localHash = require_workflow_event_processor.computeScheduleDefinitionHash(mastra.getWorkflowById(target.workflowId).serializedStepGraph);
...
//# sourceMappingURL=worker-Cfe6Sm3O.cjs.map