...
import { l as resolveExportedSpanId, p as EntityType, t as executeWithContext } from "./utils-C64E0GKG.js";
import "./tracing-D2hyX6p5.js";
import { s as RequestContext } from "./request-context-DUS79r4j.js";
import { l as getRequestContextInputValues } from "./tool-D0LLiaqQ.js";
import { toStandardSchema } from "./schema/index.js";
import { t as ToolStream } from "./stream--7NrsmVY.js";
import { v as removeUndefinedValues } from "./utils-CyZ0Xi-1.js";
import { t as TripWire } from "./trip-wire-EBUkLDk_.js";
...
const RESUME_SNAPSHOT_WAIT_STATUSES = /* @__PURE__ */ new Set(["running", "pending"]);
async function waitForSuspendedSnapshot(workflowsStore, workflowName, runId, { timeoutMs = RESUME_SNAPSHOT_POLL_TIMEOUT_MS, missingSnapshotGraceReads = 1 } = {}) {
	...
	while (Date.now() < deadline) {
		...
		} else if (!observedTransitionableSnapshot && ++missingReads >= missingSnapshotGraceReads) return null;
...
async function runAgentEntry(entry, ctx, mastra) {
	...
	const handleFinish = (result) => {
		...
		finishResult = result;
		if (agentOptions?.structuredOutput?.schema && resultWithObject.object !== void 0) {
	...
	if (agentOptions?.structuredOutput?.schema && !structuredResultProduced) throw new MastraError({
	...
	if (structuredResultProduced) return structuredResult;
...
//#endregion
//#region src/workflows/scheduler/definition-hash.ts
/**
...
*/
function computeScheduleDefinitionHash(serializedStepGraph) {
	if (serializedStepGraph == null) return void 0;
	try {
		const json = JSON.stringify(serializedStepGraph);
		if (!json || json === "[]" || json === "{}") return void 0;
	...
	*/
	...
	async execute(params) {
		...
		try {
			...
			if (stateDelta) finalResult.__stateDelta = stateDelta;
...
function isQueuedForeachIteration(value) {
...
async function processWorkflowForEach({ workflowId, prevResult, runId, executionPath, stepResults, activeStepsPath, resumeSteps, timeTravel, restart, resumeData, parentWorkflow, requestContext, perStep, state, outputOptions, forEachIndex }, { pubsub, mastra, step }) {
	...
	if (timeTravel && (!Array.isArray(currentResult?.output) || currentResult.output.length === 0) && Array.isArray(preservedForeachOutput) && preservedForeachOutput.length > 0) {
		currentResult.output = preservedForeachOutput.map((iterationResult) => iterationResult?.status === "success" ? iterationResult.output : { [FOREACH_QUEUED]: true });
		await workflowsStore?.updateWorkflowResults({
			workflowName: workflowId,
			runId,
			stepId,
	...
	const queuedIndices = Array.isArray(currentResult?.output) ? currentResult.output.flatMap((result, index) => isQueuedForeachIteration(result) ? [index] : []) : [];
	if (queuedIndices.length > 0) {
		const concurrency = resolveForeachConcurrency(step.opts, {
			inputData: prevResult?.output,
			getInitData: () => stepResults?.input
		});
		const runningCount = currentResult.output.filter((result) => result === null).length;
		const indicesToRun = queuedIndices.slice(0, Math.max(0, concurrency - runningCount));
		if (indicesToRun.length > 0) {
			const updatedOutput = [...currentResult.output];
			for (const index of indicesToRun) updatedOutput[index] = createPendingMarker();
			await workflowsStore?.updateWorkflowResults({
				workflowName: workflowId,
				runId,
				stepId,
	...
	if (idx >= targetLen && currentResult?.output?.filter((r) => r !== null && !isQueuedForeachIteration(r))?.length >= targetLen || prevResult?.output?.length === 0) {
...
var WorkflowEventProcessor = class WorkflowEventProcessor extends EventProcessor {
	...
	*/
	async #ensureScheduledDefinitionMatches(data, workflow) {
		const expected = data?.scheduleDefinitionHash;
		...
		const { workflowId, runId } = data;
		this.mastra.getLogger()?.error?.("Refusing scheduled workflow fire: local definition does not match the schedule row. This instance is running a different build of the workflow.", {
			workflowId,
			runId,
			expectedDefinitionHash: expected,
			localDefinitionHash: localHash
		});
		try {
			const schedulesStore = await this.mastra.getStorage()?.getStore("schedules");
			const match = /^sched_(.+)_(\d+)$/.exec(runId);
			if (schedulesStore && match) await schedulesStore.recordTrigger({
				scheduleId: match[1],
				runId,
			...
			this.mastra.getLogger()?.warn?.("Failed to record stale-definition schedule trigger", {
				runId,
				error: err
			});
		}
	...
	async endWorkflow(args, status = "success") {
		...
		else if (finalStatus !== "paused") try {
	...
	*/
	async aggregateBranchResults({ workflow, workflowId, runId, branchEntry, branchExecutionPath, latestBranchResult, resumeSteps, timeTravel, restart, parentWorkflow, stepResults, activeStepsPath, requestContext, state, outputOptions }) {
		...
		branchEntry.steps.forEach((branch, idx) => {
			...
			const stateDelta = idx === finishedBranchIdx && latestBranchResult?.__stateDelta ? latestBranchResult.__stateDelta : res.__stateDelta;
			if (stateDelta) Object.assign(currentState, stateDelta);
		...
		const cleanedStepResults = {
			...stepResults,
			__state: currentState
		};
		for (const [key, res] of Object.entries(cleanedStepResults)) if (res && typeof res === "object" && "__stateDelta" in res) {
	...
	async processWorkflowStepEnd({ workflow, workflowId, runId, executionPath, resumeSteps, timeTravel, restart, prevResult, parentWorkflow, stepResults, activeStepsPath, parentContext, requestContext, perStep, state, outputOptions, forEachIndex, nestedRunId }) {
		...
		const { __state: _removedState, __stateDelta: extractedStateDelta, ...cleanPrevResult } = prevResult;
		const rawStep = workflow.stepGraph[executionPath[0]];
		const branchStateDelta = (rawStep?.type === "parallel" || rawStep?.type === "conditional") && executionPath.length > 1 ? extractedStateDelta : void 0;
		...
		if (step.type === "foreach") {
			...
			if (currentIdx !== void 0) {
				...
				const existingSuspendPayload = existingStepResult?.suspendPayload;
				const iterationSuspendPayload = prevResult.suspendPayload;
				const foreachOutput = [...existingSuspendPayload?.__workflow_meta?.foreachOutput ?? []];
				foreachOutput[currentIdx] = prevResult.status === "suspended" ? prevResult : {
					...prevResult,
					suspendPayload: {}
				};
			...
			const storedResult = branchStateDelta ? {
				...prevResult,
				__stateDelta: branchStateDelta
			} : prevResult;
			const newStepResults = await workflowsStore?.updateWorkflowResults({
				workflowName: workflow.id,
				runId,
	...
	*/
	...
	async #dispatch(event) {
		...
		if (type === "workflow.start" && workflow && !await this.#ensureScheduledDefinitionMatches(data, workflow)) return;
...
//#endregion
export { validateStepInput as A, getStepIds as C, omitPriorSuspensionFields as D, omitPriorCompletionFields as E, waitForSuspendedSnapshot as F, getEntryId as I, getEntryWorkflow as L, validateStepResumeData as M, validateStepStateData as N, resolveForeachConcurrency as O, validateStepSuspendData as P, forwardAgentStreamChunk as R, getSingleStepEntryId as S, isSingleStepEntry as T, cleanStepResult as _, getTextDeltaFromChunk as a, createTimeTravelExecutionParams as b, getStepId as c, validateTemplate as d, runToolEntry as f, assertTimeTravelGraphMatchesSnapshot as g, abortableSleep as h, createTripWireFromChunk as i, validateStepRequestContext as j, runCountDeprecationMessage as k, runMappingEntry as l, getStepResult as m, StepExecutor as n, isTripwireChunk as o, runAgentEntry as p, computeScheduleDefinitionHash as r, getStepEntry as s, WorkflowEventProcessor as t, collectTemplateStepIds as u, createDeprecationProxy as v, hydrateSerializedStepErrors as w, getResumeLabelsByStepId as x, createRestartExecutionParams as y };

//# sourceMappingURL=workflow-event-processor-BSZt3kfu.js.map