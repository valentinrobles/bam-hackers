...
require("./tracing-BacWgQDZ.cjs");
const require_request_context = require("./request-context-BYAqdgDG.cjs");
const require_tool = require("./tool-4SexKovc.cjs");
const require_workspace_skills = require("./workspace-skills-C-W-_-1R.cjs");
const require_errors = require("./errors-By6VuvJ1.cjs");
...
var LocalProcessManager = class extends SandboxProcessManager {
	async spawn(command, options = {}) {
		...
		const directShell = isWindows && this.sandbox.isolation === "none" && invocation !== void 0 && /^(sh|sh\.exe)$/i.test(path.win32.basename(invocation.command)) && invocation.args[0] === "-c" && invocation.args.length >= 2;
...
* ```
*/
var MastraSandbox = class MastraSandbox extends require_base.MastraBase {
...
var LocalSandbox = class LocalSandbox extends MastraSandbox {
	...
	*/
	...
	constructor(options = {}) {
		...
		this.setWorkingDirectory(require_workspace_skills.expandTilde(options.workingDirectory ?? path.join(process.cwd(), ".sandbox")));
		this.env = options.env ?? {};
	...
	*/
	async find() {
		try {
			await fs_promises.stat(this.workingDirectory);
...
var Workspace = class {
	...
	constructor(config) {
		...
		if (config.bm25 || config.vectorStore && config.embedder) {
			...
			this._searchEngine = new require_workspace_skills.SearchEngine({
	...
	*/
	async batchReadFiles(files) {
		...
		const fs = this._fs;
		return require_workspace_skills.pMap(files, async (filePath) => {
			try {
				const content = await fs.readFile(filePath, { encoding: "utf-8" });
				const chunks = require_workspace_skills.splitIntoChunks(content);
				return {
					filePath,
	...
	*/
	async indexFilesForSearch(paths) {
		...
		try {
			...
			await require_workspace_skills.pMap(entries, ({ filePath }) => engine.removeSource(filePath), { concurrency: FS_READ_CONCURRENCY });
...
function requireComputer(context) {
	...
	if (!supportsComputer(sandbox)) throw new require_errors.SandboxFeatureNotSupportedError("computer");
	return {
		workspace,
		sandbox,
...
const computerClickTool = require_tool.createTool({
	...
	inputSchema: zod_v4.z.object({
		x: zod_v4.z.number().int().min(0).describe("X coordinate in pixels from the left edge of the screen"),
		y: zod_v4.z.number().int().min(0).describe("Y coordinate in pixels from the top edge of the screen")
	}),
...
const computerDoubleClickTool = require_tool.createTool({
...
const computerDragTool = require_tool.createTool({
	...
	inputSchema: zod_v4.z.object({
		startX: zod_v4.z.number().int().min(0).describe("X coordinate to start the drag from"),
		startY: zod_v4.z.number().int().min(0).describe("Y coordinate to start the drag from"),
		endX: zod_v4.z.number().int().min(0).describe("X coordinate to drag to"),
		endY: zod_v4.z.number().int().min(0).describe("Y coordinate to drag to")
	}),
...
const computerGetScreenInfoTool = require_tool.createTool({
...
const computerMoveMouseTool = require_tool.createTool({
...
const computerPressKeyTool = require_tool.createTool({
	...
	inputSchema: zod_v4.z.object({ key: zod_v4.z.union([zod_v4.z.string().min(1), zod_v4.z.array(zod_v4.z.string().min(1)).min(1)]).describe("A single key (e.g. \"Enter\") or an array of keys pressed together as a hotkey (e.g. [\"ctrl\", \"c\"])") }),
...
const computerRightClickTool = require_tool.createTool({
...
const computerScreenshotTool = require_tool.createTool({
...
const computerScrollTool = require_tool.createTool({
	id: WORKSPACE_TOOLS.COMPUTER.SCROLL,
	description: "Scroll the sandbox desktop display up or down.",
	inputSchema: zod_v4.z.object({
		direction: zod_v4.z.enum(["up", "down"]).describe("Scroll direction"),
		amount: zod_v4.z.number().int().min(1).optional().describe("Scroll amount in ticks (default: 3)")
	}),
...
const computerTypeTool = require_tool.createTool({
	...
	inputSchema: zod_v4.z.object({ text: zod_v4.z.string().describe("The text to type") }),
...
const computerWaitTool = require_tool.createTool({
	...
	inputSchema: zod_v4.z.object({ seconds: zod_v4.z.number().min(.1).max(30).describe("How long to wait, in seconds (max 30)") }),
	...
	*/
	injectCdpUrl(command, cdpUrl, threadId) {
		...
		if (headerParts.parts.some((part) => this.isBrowserUseStdinCommand(part))) {
			...
			const name = (0, crypto.createHash)("sha256").update(JSON.stringify([threadId ?? "default", cdpUrl])).digest("hex").slice(0, 16);
...
function sandboxToModelOutput(output) {
	...
	if (require_tool.isValidationError(output)) return {
		type: "error-json",
		value: output
	};
	return output;
...
async function applyTokenLimit(output, limit = DEFAULT_MAX_OUTPUT_TOKENS, from = "start") {
	...
	const kept = (from === "start" ? (0, tokenx.sliceByTokens)(output, -limit) : (0, tokenx.sliceByTokens)(output, 0, limit)).replace(UNPAIRED_SURROGATE_RE, "�");
...
async function applyTokenLimitSandwich(output, limit = DEFAULT_MAX_OUTPUT_TOKENS, headRatio = .1) {
	...
	const head = headBudget > 0 ? (0, tokenx.sliceByTokens)(output, 0, headBudget).replace(UNPAIRED_SURROGATE_RE, "�") : "";
	const tail = tailBudget > 0 ? (0, tokenx.sliceByTokens)(output, -tailBudget).replace(UNPAIRED_SURROGATE_RE, "�") : "";
	return [
		head,
...
const executeCommandInputSchema = zod_v4.z.object({
	...
	tail: zod_v4.z.preprocess(coerceNumericString, zod_v4.z.number().int()).nullish().describe(`For foreground commands: limit output to the last N lines, similar to tail -n. Defaults to 200. Use 0 for no limit.`)
});
...
Use this after starting a background command with execute_command (background: true) to check if the process is still running and read its output.`,
	...
	inputSchema: zod_v4.z.object({
		...
		tail: zod_v4.z.preprocess(coerceNumericString, zod_v4.z.number().int()).optional().describe(`Number of lines to return, similar to tail -n. Positive or negative returns last N lines from end. Defaults to 200. Use 0 for no limit.`),
	...
	execute: async ({ pattern, path: inputPath = ".", contextLines = 0, maxCount, caseSensitive = true, includeHidden = false }, context) => {
		...
		try {
			...
			if (normalizedSearch === ".git" || normalizedSearch.endsWith("/.git")) filePaths = [];
				...
				if (stat.type === "file") filePaths = require_workspace_skills.isTextFile(searchPath) ? [searchPath] : [];
					...
					if (error instanceof require_errors.UnsupportedGrepPatternError) workspace.logger?.info(`Native grep on "${filesystem.provider}" filesystem does not support pattern "${pattern}"; falling back to host-side search`);
				...
				if (stat.type !== "file" && !nativeResults) {
					...
					while (directoryFrontier.length > 0) {
						const directoryEntries = await require_workspace_skills.pMap(directoryFrontier, async (dir) => {
							try {
			...
			*/
			...
			if (nativeResults) for (const result of nativeResults) {
				...
				if (!require_workspace_skills.isTextFile(segments[segments.length - 1])) continue;
			...
			for (let batchStart = 0; batchStart < filePaths.length && !truncated; batchStart += GREP_FILESYSTEM_CONCURRENCY) {
				...
				const batchContents = await require_workspace_skills.pMap(batchPaths, async (filePath) => {
					try {
...
async function formatAsTree(fs, path, options) {
	...
	*/
	...
	if (shouldWalk && typeof fs.walk === "function") try {
		...
		entriesByPath.clear();
		if (!(error instanceof require_errors.DirectoryNotFoundError) && !(error instanceof require_errors.NotDirectoryError)) options?.logger?.warn(`Native walk failed on "${fs.provider}" filesystem; falling back to per-directory readdir walk for "${path}"`, { error: error instanceof Error ? error.message : String(error) });
	}
	...
	while (frontier.length > 0) {
		const results = await require_workspace_skills.pMap(frontier, async (dir) => {
			try {
...
Object.defineProperty(exports, "MAX_SANDBOX_FILE_MODE", {
	enumerable: true,
	get: function() {
		return MAX_SANDBOX_FILE_MODE;
	}
});
...
Object.defineProperty(exports, "SandboxAbortError", {
	enumerable: true,
	get: function() {
		return SandboxAbortError;
	}
});
...
Object.defineProperty(exports, "SandboxUnsupportedFeatureError", {
	enumerable: true,
	get: function() {
		return SandboxUnsupportedFeatureError;
	}
});
...
Object.defineProperty(exports, "assertModesUnsupported", {
	enumerable: true,
	get: function() {
		return assertModesUnsupported;
	}
});
Object.defineProperty(exports, "browserCliHandler", {
	enumerable: true,
...
Object.defineProperty(exports, "supportsComputer", {
	enumerable: true,
	get: function() {
		return supportsComputer;
	}
});
...
Object.defineProperty(exports, "validateSandboxFileMode", {
	enumerable: true,
	get: function() {
		return validateSandboxFileMode;
	}
});
...
//# sourceMappingURL=workspace-BmarCUep.cjs.map