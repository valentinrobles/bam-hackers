...
//#region src/upstash/index.ts
var UpstashTransport = class extends _mastra_core_logger.LoggerTransport {
	upstashUrl;
	upstashToken;
	listName;
	maxListLength;
	...
	constructor(opts) {
		...
		if (!opts.upstashUrl || !opts.upstashToken) throw new Error("Upstash URL and token are required");
	...
	async executeUpstashCommand(command) {
		...
		if (!response.ok) throw new Error(`Failed to execute Upstash command: ${response.statusText}`);
	...
	async _flush() {
		...
		try {
			...
			if (this.maxListLength > 0) command.push("LTRIM", this.listName, 0, this.maxListLength - 1);
	...
	async listLogs(params) {
		try {
			...
			let filteredLogs = ((await this.executeUpstashCommand(command))?.[0]?.result?.map((log) => {
				try {
			...
			const { fromDate, toDate, logLevel, filters, returnPaginationResults: returnPaginationResultsInput, page: pageInput, perPage: perPageInput } = params || {};
			...
			if (filters) filteredLogs = filteredLogs.filter((log) => Object.entries(filters || {}).every(([key, value]) => log[key] === value));
			if (logLevel) filteredLogs = filteredLogs.filter((log) => log.level === logLevel);
			if (fromDate) filteredLogs = filteredLogs.filter((log) => new Date(log.time).getTime() >= fromDate.getTime());
			if (toDate) filteredLogs = filteredLogs.filter((log) => new Date(log.time).getTime() <= toDate.getTime());
			if (!returnPaginationResults) return {
				logs: filteredLogs,
	...
	async listLogsByRunId({ runId, fromDate, toDate, logLevel, filters, page: pageInput, perPage: perPageInput }) {
		try {
			const page = pageInput === 0 ? 1 : pageInput ?? 1;
			const perPage = perPageInput ?? 100;
			const logs = (await this.listLogs({
				...
				logLevel,
				filters,
				returnPaginationResults: false
			}))?.logs?.filter((log) => log.runId === runId) || [];
