...
function resolveTaskMemory({ task, agentId, requestContext, metadata, message }) {
	const trustedResource = usableId(requestContext?.get(_mastra_core_request_context.MASTRA_RESOURCE_ID_KEY));
	const trustedThread = usableId(requestContext?.get(_mastra_core_request_context.MASTRA_THREAD_ID_KEY));
	const storedResource = usableId(task?.metadata?.resourceId);
	if (task && (trustedResource && storedResource && trustedResource !== storedResource || trustedThread && trustedThread !== task.contextId)) throw _mastra_core_a2a.MastraA2AError.invalidRequest("Task memory identity conflicts with the authenticated request.");
	return {
...
//# sourceMappingURL=a2a-DQ8GeDz4.cjs.map