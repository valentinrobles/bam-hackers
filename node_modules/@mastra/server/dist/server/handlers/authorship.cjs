...
//#region src/server/handlers/authorship.ts
/**
...
function getCallerAuthorId(requestContext) {
	const resourceId = requestContext.get(require_constants.MASTRA_RESOURCE_ID_KEY);
...
function getCallerPermissions(requestContext) {
	const raw = requestContext.get(require_constants.MASTRA_USER_PERMISSIONS_KEY);
...
function hasScopedPermission(args) {
	...
	if (!resourceId) {
		...
		return permissions.some((p) => (0, _mastra_core_auth_ee.matchesPermission)(p, required));
	}
	const required = `${resource}:${action}:${resourceId}`;
	return permissions.some((p) => {
		if (p.split(":").length < 3) return false;
		return (0, _mastra_core_auth_ee.matchesPermission)(p, required);
	});
}
/**
...
function resolveAuthorFilter(args) {
	...
	if (queryVisibility === "public" && !queryAuthorId) return { kind: "publicOnly" };
	if (bypass) return queryAuthorId ? {
		kind: "exact",
		authorId: queryAuthorId
	} : { kind: "unrestricted" };
	if (!callerAuthorId) return queryAuthorId ? {
		kind: "exact",
		authorId: queryAuthorId
	} : { kind: "unrestricted" };
	if (queryAuthorId) {
		if (queryAuthorId === callerAuthorId) return {
			kind: "exact",
...
function matchesAuthorFilter(record, filter) {
	...
	switch (filter.kind) {
		...
		case "exact": return owner === filter.authorId;
		case "ownedOrPublic": return owner === null || owner === filter.callerAuthorId || isPublic;
		case "publicOnly": return owner === null || isPublic;
		case "ownedOrPublicOthers": return owner === filter.queryAuthorId && isPublic;
	}
}
/**
...
function assertReadAccess(args) {
	...
	const callerAuthorId = getCallerAuthorId(requestContext);
	if (!callerAuthorId && !requestContext.get("mastra__user")) return;
