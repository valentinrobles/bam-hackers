...
//#region src/server/handlers/skills-sh-shared.ts
/**
...
const PREVIEW_TIMEOUT_MS = 1e4;
const FILE_FETCH_TIMEOUT_MS = 3e4;
/**
...
function assertSafeFilePath(filePath) {
	if (filePath.startsWith("/") || filePath.startsWith("\\") || /^[a-zA-Z]:[\\/]/.test(filePath)) throw new HTTPException(400, { message: `Invalid file path "${filePath}". Absolute paths are not allowed.` });
...
async function getPopularSkillsSh({ limit, offset }) {
	const controller = new AbortController();
	const timeoutId = setTimeout(() => controller.abort(), SEARCH_TIMEOUT_MS);
	try {
...
*/
async function previewSkillsSh({ owner, repo, skillName }) {
	const controller = new AbortController();
	...
	try {
		...
		const response = await fetch(url, { signal: controller.signal });
		if (!response.ok) {
			if (response.status === 404) throw new HTTPException(404, { message: `Could not find skill "${skillName}" for ${owner}/${repo}` });
			throw new HTTPException(502, { message: `Skills API error: ${response.status} ${response.statusText}` });
		}
		const data = await response.json();
...
//# sourceMappingURL=skills-sh-shared.js.map