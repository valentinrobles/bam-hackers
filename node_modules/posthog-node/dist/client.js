...
class PostHogBackendClient extends core_namespaceObject.PostHogCoreStateless {
    ...
    _flushEventsAndSpans(skipThrottledSpans = false) {
        ...
        if (!this._traces || skipThrottledSpans && this._traces.throttled) return events;
        return (0, core_namespaceObject.allSettled)([
            events,
            this._traces.flush().catch(()=>{})
        ]).then(([eventsResult])=>{
            if ('rejected' === eventsResult.status) throw eventsResult.reason;
        });
    }
    flushAutomatic() {
    ...
    initializeSpanContextManager() {
        return new core_namespaceObject.SyncSpanContextManager();
    }
    ...
    get _tracesPipeline() {
        if (!this.options.traces) return;
        if (!this._traces) this._traces = new core_namespaceObject.PostHogTraces(this, (0, core_namespaceObject.resolveTracesConfig)(this.options.traces, this.hostResourceAttributes(), this._logger), this._logger, ()=>this._tracingContext(), this._spanContextManager, ()=>this.scheduleDebouncedFlush());
        return this._traces;
    }
    _tracingContext() {
    ...
    startSpan(name, options) {
        return this._tracesPipeline?.startSpan(name, options) ?? (0, core_namespaceObject.inertSpan)(options, this._spanContextManager.active());
    }
    withSpan(name, optionsOrFn, maybeFn) {
        ...
        if (!pipeline) return (0, core_namespaceObject.runWithActiveSpan)(this._spanContextManager, (0, core_namespaceObject.inertSpan)(options, this._spanContextManager.active()), fn);
    ...
    async _shutdown(shutdownTimeoutMs) {
        ...
        if (this._traces) {
            await (0, core_namespaceObject.raceWithTimeout)(this._traces.flush().catch(()=>{}), Math.max(0, shutdownDeadlineMs - Date.now()));
            this._traces.reset();
        }
        try {
