...
import { PostHogCoreStateless, PostHogMetrics, PostHogPersistedProperty, PostHogTraces, SyncSpanContextManager, allSettled, getEventUuid, inertSpan, isBlockedUA, isPlainObject, isPostHogFetchContentTooLargeError, minimizeFlagCalledEventProperties, raceWithTimeout, resolveMetricsConfig, resolveTracesConfig, runWithActiveSpan, safeSetTimeout, uuidv7 } from "@posthog/core";
...
class PostHogBackendClient extends PostHogCoreStateless {
    ...
    get _tracesPipeline() {
        ...
        if (!this._traces) this._traces = new PostHogTraces(this, resolveTracesConfig(this.options.traces, this.hostResourceAttributes(), this._logger), this._logger, ()=>this._tracingContext(), this._spanContextManager, ()=>this.scheduleDebouncedFlush());
        return this._traces;
    }
    _tracingContext() {
    ...
    withSpan(name, optionsOrFn, maybeFn) {
        ...
        if (!pipeline) return runWithActiveSpan(this._spanContextManager, inertSpan(options, this._spanContextManager.active()), fn);
