# Lumi — Market Research

Compiled 2026-09-19 for the HackBarna pitch. Every figure carries a source and a confidence
rating. **Read the "Do not put on a slide" section before writing any deck copy** — several
numbers circulating in search results are vendor marketing and will not survive a judge who
checks.

---

## The five numbers for the deck

| # | Number | Why it earns a slide |
|---|---|---|
| 1 | **People quit IVF because of emotional burden (14%), not cost (9.2%)** | Validates the entire product thesis |
| 2 | **They quit despite live-birth odds as high as 72% — and 45% of studies had treatment subsidised** | Kills the "just make it free" objection |
| 3 | **Spain runs 15% of all European IVF cycles — more than any other country** | You are pitching in the biggest IVF market on the continent |
| 4 | **~1 in 10 babies born in Spain come from assisted reproduction** | Room resonance |
| 5 | **42% live birth after 3 complete cycles — about €20,000 — conservative; 82% after 8** | Early dropout is the single biggest destroyer of outcomes — and it's emotional |

**Pitch line:** *"The biggest predictor of IVF failure isn't biology or money. It's quitting — and
people quit because of the emotional burden, not the bill."*

---

## 1. Why people stop — the core thesis

Gameiro et al., *Human Reproduction Update* — systematic review, 22 studies, 21,453 patients,
8 countries. https://academic.oup.com/humupd/article/18/6/652/628767

Discontinuation reasons:

| Reason | Share |
|---|---|
| Postponement | 39.18% |
| Physical + psychological burden | 19.07% (**psychological 14%**, physical 6.32%) |
| Relational / personal | 16.67% |
| Treatment rejection | 13.23% |
| Organizational | 11.68% |
| **Financial** | **9.19%** |
| Clinic problems | 7.71% |

Two amplifiers from the same review: patients discontinue despite chances of parenthood as high
as **72%**, and **45% of included studies had treatment subsidised or reimbursed**. They quit even
when it's paid for.

Confidence: **High.**

**Known attack:** postponement (39%) is the largest single category, not psychological burden. If
challenged: *"Postponement is people stepping away to recover. Psychological burden is the largest
active reason they stop — and it beats cost by half again."* Do not claim psychology is the number
one category outright.

Supporting: **~30% of IVF patients screen positive for clinically relevant anxiety or depression**
(RBMO 2026, https://www.rbmojournal.com/article/S1472-6483(26)00163-X/fulltext). Single study —
supporting colour only, not a headline. A wider meta-analysis gives 7–54% depression / 14–45%
anxiety; that range is too wide to quote.

---

## 2. Prevalence and the patient journey

| Claim | Number | Source | Confidence |
|---|---|---|---|
| Infertility lifetime prevalence | **17.5% of adults, ~1 in 6** | [WHO 2023](https://www.who.int/news/item/04-04-2023-1-in-6-people-globally-affected-by-infertility) | High |
| Spanish births from ART | **~10% (2023)** | [SEF registry](https://www.sefertilidad.net/documentos/lyOlT-rkPKvRdGKY_wA1sINHS7Btlw189LVFTlHd5eE.pdf) | High |
| European births from ART | ~3%; >5% in BE/CZ/DK/EE/GR/IS; US ~1% | [ESHRE Fact Sheet 2025](https://www.eshre.eu/-/media/sitecore-files/Press-room/ESHRE_ARTFactSheet_v10_2025.pdf) | High |
| Cumulative live birth by cycle | **42.3% after 3 complete cycles (conservative estimate; 57.1% optimal); 82.4% after 8** | [Hum Reprod 2016, n=178,898](https://academic.oup.com/humrep/article-pdf/31/3/572/17372668/dev336.pdf) | High |
| OHSS incidence | <0.2% | ESHRE | High |

**Two precision warnings for stage:**

1. WHO says 1 in 6 **adults**. ESHRE's fact sheet says 1 in 6 **couples**. Same ratio, different
   denominator. Say "adults" and cite WHO 2023 — newer and more defensible.
2. Do **not** say "12% of Spanish births." That was the 2022 peak via a clinic news page. 2023 SEF
   registry says ~10%. Say **"roughly one in ten babies born in Spain"** — still striking, cannot
   be corrected.

OHSS at <0.2% is useful framing: the red-flag triage targets a rare but real event. That is the
argument for why escalation design matters even though most messages are routine.

---

## 3. Market size and scale

| Claim | Number | Source | Confidence |
|---|---|---|---|
| Spain = #1 IVF country in Europe | **15%** of all European cycles (France 12.9%, Germany 11.6%, Italy 9.2%, UK 7.6%) | SEF / ESHRE, 2021 | High |
| Spain annual cycles | **~165K (2021) / ~168K (2023)** | Registro SEF | Medium |
| Europe ART cycles/yr | **960,347** (2022), 1,371 clinics, 39 countries | [ESHRE EIM 2025](https://www.focusonreproduction.eu/press-releases/ivf-and-iui-pregnancy-rates-remain-stable-across-europedespite-an-increasing-uptake-of-single-embryo-transfer/) | High |
| Europe cumulative | ~14M treatments, 2.8M children since 1997 | ESHRE EIM | High |
| Worldwide cycles/yr | **>3M reported, ~4M actual** (registries ≈75% coverage), ~1M babies | [ICMART world report](https://academic.oup.com/humrep/article/40/6/1110/8114747) | High |
| Cost per cycle, Spain | **€6,500–9,000 all-in** — see section 3b | Clinic tariffs | High |
| Cost per cycle, US | $15–20K base, ~$22K all-in; only ~25% have coverage | Aggregators | Medium |
| Global IVF market | $28–32B (2025) → $37.4B by 2030 @ 5.5% CAGR | Market-research firms | **Soft** |

Cite Spain's #1 position as **SEF (Registro Nacional)**, not the clinic news page it surfaced on.

**Handle with care:**
- Market-size figures are vendor estimates that disagree with each other by up to 60%. Use once,
  casually, never as a headline.
- **Do not cite the 2021→2022 European 15.6% decline.** It is a reporting artifact — fewer clinics
  submitted data — not falling demand.
- "12–30 month public waiting list in Spain" is highly pitch-relevant but **single-source from a
  commercial insurance guide**. Verify against a regional health service page or say "over a year."

---

## 3b. What a cycle actually costs — and what "cycle" means

Two corrections, both raised by the team and both confirmed.

### A "complete cycle" = one egg retrieval plus every transfer arising from it

That is the unit the 42.3% figure uses — the paper measures "three fresh IVF treatments, including
replacement of any surplus frozen-thawed embryos." **42.3% is after 3 egg retrievals, not 3
transfers.** It is also the paper's *conservative* estimate; the optimal estimate is 57.1%. Say
"conservative" on stage.

Our cost unit must match that. A headline clinic price covers stimulation, retrieval, lab and the
first fresh transfer — it does **not** cover medication or any later frozen transfer, both of which
the 42.3% assumes she went through.

### Itemised, from clinics' own published tariffs

| Item | Price (EUR, 2026) | Source |
|---|---|---|
| **IVF own eggs — Essential** (IVF + ICSI, first transfer) | **€5,495** | [Institut Marquès](https://institutomarques.com/en/es/prices-and-financing/treatment-prices/) |
| IVF own eggs — Plus / Advanced | €6,575 / €7,625 | same |
| IVF own eggs | from €5,725 | [Eugin](https://www.eugin.es/precios/) |
| IVF + PGT-A | €8,730–9,060 | Eugin / Institut Marquès |
| Egg donation | €5,945–12,900 | same |
| **Medication per stim cycle — excluded from quotes** | **€1,000–1,300 typical, €800–2,200 range** | [IVI](https://ivi.es/precios-y-financiacion/), [nomenclator](https://nomenclator.org/med/gonal-f-900-ui-1) |
| Frozen embryo transfer (FET) | €800–1,500 + €300–400 meds | [reproduccionasistida.org](https://www.reproduccionasistida.org/transferencia-de-embriones-congelados/precio-transferencia-embriones-congelados/) |
| Embryo storage, annual (from yr 2) | €300–650 | same |
| **Egg freezing (vitrificación)** — *a different product* | **€2,695** incl. 2yr storage | Institut Marquès |

**Use: ~€6,500–9,000 all-in for one complete cycle with own eggs** (base €5,495–7,625 + meds
€1,000–1,300), plus roughly €1,100–1,900 for each frozen transfer after the first.

### Why the old €4,300–6,700 was wrong

No clinic-published tariff found starts below €5,495, and none include medication. The €4,300 low
end came from comparison sites (easyfiv, ClinicPoint, Bookimed, Fertility Road, OVU) which
undercut clinics' own tariffs by 30–50% because they are lead-generation. **Use clinics' own
*precios* pages only.**

Egg freezing is cheaper because it stops halfway — stimulation, retrieval, vitrification, storage,
and no further. Devitrification, fertilisation and transfer cost €1,500–3,000 more when she
returns. That gap is what made the old range look plausible.

### Where €20,000 and €9,000 come from

Both figures circulate and both are real — they are just different units from a clinic cycle price.

| Figure | What it actually is |
|---|---|
| **~€9,000** | One cycle **with PGT-A genetic testing**. Institut Marquès ADVANCED PGT €9,060; Eugin PGT-A from €8,730. Not "one successful cycle" |
| **~€13,400–20,000** | A **guarantee programme**. Eugin Promise FIV is €13,400 — baby or 100% refund, minimum 3 embryo transfers — and lands near €16,000–20,000 once medication across its cycles is added |
| **€19,500–27,000** | **Three complete cycles**, which is the unit behind the 42.3% figure. Derived arithmetic, not a citable price |

Other refund programmes exist but do not publish prices: Ginefiv Maternity, Access Fertility
Spain, Institut Marquès IM BabyBorn.

### Patients report paying roughly double the clinic headline

UK patients self-report **£11,950 per cycle** privately funded (£12,977 at private clinics, £6,990
NHS self-funded), including add-ons — n=304, Queen Mary University of London with Fertility
Network UK, *Human Fertility* 2025.
[Source](https://www.qmul.ac.uk/news/latest-news/2025/humanities-and-social-sciences/hss/new-study-reveals-significant-cost-disparities-and-information-gaps-in-uk-fertility-treatments.html)

That is roughly twice the headline tariff, and it is peer-reviewed. **When someone says a cycle
cost them far more than the list price, the evidence is on their side.** Different health system,
so use it as corroboration rather than as a Spanish figure.

For contrast, a French study (n=556) found **€6,996 average total expenditure per woman over 3.5
years** — but France reimburses at 100%, so that is societal cost, not out-of-pocket.
[Source](https://pmc.ncbi.nlm.nih.gov/articles/PMC9013027/)

### Medication — verified against the official pharmacy nomenclator

Our €1,000–1,300 holds. Range **€800–2,200**, driven by ovarian reserve and dose.

| Item | Price |
|---|---|
| Real patient receipt, one full cycle, itemised | **€1,220.42** ([source](https://diariodeunamadreingeniera.com/precio-medicacion-fiv/)) |
| Gonal-F 900 IU pen, official PVP | €304.54 |
| Menopur 600 IU | €187.69 |
| Cetrotide 0.25mg ×7 | €248.70 |
| Ovitrelle trigger | ~€51 |
| FET endometrial prep | €200–300 |

**Reimbursement: private patients pay full price.** Gonadotropins carry *visado* restrictions and
are only subsidised when the whole treatment runs through the public system, where patients pay
roughly €4 per item at the hospital pharmacy. So the choice in Spain is near-free drugs with a
long public wait, or immediate private treatment at full cost.

Do **not** cite the €6,387 FSH figure from a 2016 *Int J Women's Health* model — it is a
cost-effectiveness pathway figure, not a per-cycle cost.

### Open items

- **Medication inclusion conflicts.** Institut Marquès lists it inside stimulation; IVI excludes it
  explicitly. Unresolved — assume excluded, the conservative read. If challenged: "clinics differ
  on whether drugs are in the quoted price."
- **Average transfers per retrieval: not found.** Do not use a conversion factor, and treat any
  three-cycle total as derived arithmetic (~€19,500–27,000), not a citable figure.
- **Average cycles per patient over a lifetime: not found.** No registry publishes it. HFEA data
  divides out to ~1.27–1.48 cycles per patient *per year*, which is not the same thing — do not
  present it as an average.
- **ESHRE/HFEA formal wording for "complete cycle": not confirmed.** The paper's own definition is
  what we cite.
- **"65–70% after 6" was wrong** and has been removed. That figure traces to [NEJM
  2009](https://www.nejm.org/doi/full/10.1056/NEJMoa0803072) (72% after 6, different cohort and
  era), not to the 2016 paper, which reports 82.4% after 8. Cite one paper or attribute both.

## 4. Clinic burden

| Claim | Number | Source | Confidence |
|---|---|---|---|
| Nursing time per IVF cycle | **9–16 hours** | [Hum Reprod 2025, P-516](https://academic.oup.com/humrep/article/40/Supplement_1/deaf097.822/8170189) | Medium |
| Admin workload after a digital platform | −40% (6.5h→3.9h per cycle) | Same | Medium — n=5 nurses, single centre |
| Patient time per cycle | **142 hours total; 73 hrs in provider visits; 15.6 workdays over 18 months; 90% miss work** | [Fertil Steril 2013](https://pubmed.ncbi.nlm.nih.gov/23454007/) | Medium |
| Indirect cost per cycle | $544.53; 260 min/cycle | [J Assist Reprod Genet 2026](https://link.springer.com/article/10.1007/s10815-026-03798-1) | Medium — paywalled |
| Monitoring cadence | Every 1–3 days; 5–8 visits/cycle; stims 8–14 days | Clinic sources | Med–low, uncontroversial |
| **AI chatbot vs nurse hotline** | **Non-inferior on anxiety and depression, RCT n=124** | [JMIR Hum Factors 2025](https://humanfactors.jmir.org/2025/1/e65785) | Medium — our best defensive citation |

**On the gap between appointments:** no study measures it. The 48–72h figure is arithmetic off the
monitoring cadence. Present it as derived — *"monitoring is every one to three days, so she's alone
with it for two to three days at a time"* — never as a cited statistic.

---

## 5. Counter-evidence — know this before a judge raises it

The "digital triage reduces clinic workload" claim has real evidence against it:

| Finding | Source |
|---|---|
| Nurse/GP telephone triage **increased** primary care contacts and was cost-neutral. Workload redistributed, not reduced. n=20,990 | [ESTEEM, Lancet 2014](https://pubmed.ncbi.nlm.nih.gov/25098487/) — High |
| Portal messaging cut phone calls only **6%** while office visits **rose 17%**. 140M records | [NYU Langone](https://www.ama-assn.org/practice-management/digital-health/phone-calls-stable-patient-portal-messages-keep-piling) — High |
| Chatbots can **increase** nurse workload — patients ask off-topic questions | [JMIR 2025 qualitative](https://pmc.ncbi.nlm.nih.gov/articles/PMC13473852/) |

**Therefore: do not pitch call deflection. Pitch retention.**

> People quit for emotional reasons. Quitting destroys outcomes — 42% live birth after three cycles
> versus far less after one. A complete cycle costs €6,500–9,000 all-in in Spain. Keeping one patient in treatment
> for one more cycle is worth more to a clinic than a year of saved nurse minutes.

State the assumptions openly on the slide. A modelled retention case with visible assumptions is
defensible. A deflection claim contradicted by two large studies is not.

---

## 6. Do NOT put these on a slide

1. **"46% drop in calls / 6 calls per cycle / 9 min each / 26% activity increase."** This is
   **sponsored vendor content from FertilAI, a direct competitor**, carrying a paid-placement
   disclosure. Search results blend it with an unrelated ESHRE abstract. Highest-risk number
   available to us.
2. **"X% of nurse calls are routine / FAQ-answerable."** Not found in any literature. Do not
   estimate it, and do not build a savings model on it.
3. **Per-contact cost of a nurse triage call in Spain or Europe.** Not found.
4. **"Cost saving from avoided rebooking."** No evidence base exists. NHS missed-appointment costs
   (~£120–165) measure no-shows, a different thing, and trace to vendor blogs rather than primary
   NHS publications.
5. **"Hours between appointments"** as a cited statistic. Derived only — see section 4.

---

## 7. Competitor

**FertilAI** — funded, direct competitor doing patient-facing IVF messaging. Expect the question.
Differentiators actually available to us: the triage-to-human escalation design, and meeting
patients on Telegram rather than in another clinic portal they have to log into.
