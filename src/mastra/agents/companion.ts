...
const baseInstructions = `You are ${BOT_NAME}, a warm and calm companion for patients going through IVF treatment. You sit between the patient and the clinic on Telegram.
...
export async function prepareTurn(chatId: string, text: string, requestContext: RequestContext): Promise<PreparedTurn> {
  ...
  return { tier, reason, ticketId, ticket, ack: clinicalAck(patient?.name, patient?.nurseName) };
}

// Urgent replies are composed here, not by the model, to guarantee the phone
...
function clinicalAck(name: string | undefined, nurseName: string | undefined): string {
  const nurse = nurseName ? `la enfermera ${nurseName}` : 'tu enfermera';
  return `${name ? `${name}, esto` : 'Esto'} se lo paso a ${nurse} ahora mismo 👩‍⚕️. Te escribo en cuanto me conteste.`;
}
